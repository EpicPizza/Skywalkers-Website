<script lang=ts>
    ...
    import { clickOutside } from '$lib/stores';
        ...
        unsubscribeUser = client.subscribe((user) => {
            ...
            unsubscribeRoles = onSnapshot(ref, async (snapshots) => {
                ...
                for(let i = 0; i < snapshots.docs.length; i++) {
                    let members = await getUsers(snapshots.docs[i].data().members as DocumentReference[]);
    ...
    async function handleSort(e: CustomEvent) {
        ...
        for(let i = 0; i < data.roles.length; i++) { //removes old moved role
            if(data.roles[i].id === fromRole.id && (data.roles[i] as any).moved != true) {
                data.roles.splice(i, 1);
            }
        }

        for(let i = 0; i < data.roles.length; i++) { //resets moved role
        ...
        waiting = fetch("/api/roles/resort", { //actually resort in database
            method: "POST",
            headers: {
    ...
    function checkIfClickedOutside(value: boolean) {
        if((value == false || value == true) && small == true && $sidebar == true) {
            $sidebar = false;
        }
    }
    ...
    const rolesStore = writable(data.roles);

    $: $rolesStore = data.roles;

    setContext('roles', rolesStore);
    ...
    $: {
        ...
        let userLevel = $client == undefined ? undefined : !$client.permissions?.includes("MANAGE_ROLES") ? undefined : $client.level;
...
<div class="min-h-[calc(100dvh)] w-full pt-[82px] p-2 pattern">
    <div class="sm:flex min-h-[calc(100dvh-90px)] md:min-w-[768px] md:absolute md:left-1/2 md:-translate-x-1/2 rounded-xl border-[1px] border-border-light dark:border-border-dark overflow-hidden">
        <div class="{$sidebar ? "left-[7px]" : "-left-44"} rounded-l-xl -translate-y-[1px] transition-all absolute z-30 sm:relative sm:left-0 bg-backgroud-light dark:bg-backgroud-dark {small ? "border-[1px] max-h-[calc(100dvh-90px)] min-h-[calc(100dvh-90px)]" : "max-h-[calc(100dvh-91px)] min-h-[calc(100dvh-91px)] -mb-[1px]"} min-w-[11rem] max-w-[11rem] sm:m-h-full p-3 pt-0 border-border-light dark:border-border-dark border-r-[1px] flex flex-col justify-between">
            ...
            {#if loading}
                <div transition:fly={{ y: 8 }} class="flex absolute bottom-12 left-1/2 -translate-x-1/2 flex-col items-center p-2 bg-backgroud-light dark:bg-backgroud-dark border-border-light dark:border-border-dark border-[1px] rounded-md mb-2">
                    <Loading></Loading>
                </div>
            {/if}
        </div>
