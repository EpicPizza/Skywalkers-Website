<script lang=ts>
    ...
    function selectHandler(event: Event) {
        if(event.target && 'files' in event.target) {
            files.push(... event.target.files as any);
        }
...
<div class="min-h-[calc(100dvh-4rem)] p-8 flex justify-around">
    <div class="w-[36rem] lg:w-[44rem]">
        ...
        <form method=POST use:enhance>
                    ...
                    <div class="mt-2 -mb-3 text-center">
                        {#each files as file, i}
                            <div class="inline-flex bg-black dark:bg-white bg-opacity-10 dark:bg-opacity-10 p-2 px-3 rounded-lg w-fit items-center gap-2 mb-2 mr-2">
                                <p>{file.name}</p>
                                <button on:click|preventDefault={() => { files.splice(i, 1); files = files; }} class="w-6 h-6 transition bg-black dark:bg-white bg-opacity-10 dark:bg-opacity-10 hover:bg-opacity-20 hover:dark:bg-opacity-20 flex rounded-full justify-around items-center"><Icon scale=1.1rem icon=close></Icon></button>
                            </div>
                        {:else}
            ...
            <input on:change={selectHandler} hidden bind:this={filepicker} type=file multiple/>
