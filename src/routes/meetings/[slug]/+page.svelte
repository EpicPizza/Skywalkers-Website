<script lang=ts>
    ...
    onMount(() => {
        ...
        unsubscribe = client.doc<any>(doc(client.getFirestore(), "teams", $client.team, "meetings", data.meeting.id), "loading").subscribe(async (value) => {
            ...
            if(value.version != PUBLIC_MEETING_VERSION) {
                    fetch('/meetings/' + value.id + "/update");
            }
    ...
    function checkConfirmation(confirmations: Meeting["confirmations"], id: string) {
    ...
    <div class="w-[36rem] lg:w-[44rem] min-h-full relative">
        ...
        {:else}
            ...
            <div class="p-4 lg:p-6 bg-accent-500 text-black rounded-2xl flex items-center">
                ...
                <button on:click={() => { navigator.clipboard.writeText('/meeting id:' + data.meeting.id); warning.set({ color: 'aqua', message: "Discord Slash Command Copied" }) }} class="p-2 rounded-full bg-accent-light dark:bg-accent-dark text-accent-text-light dark:text-accent-text-dark hover:brightness-95 transition">
                    <Icon icon=share scale=2rem class="-translate-x-0.5"/>
                </button>
            </div>
            {#if data.meeting.confirmations && signedup && !(checkConfirmation(data.meeting.confirmations, $client?.uid ?? "000000") === undefined)}
                ...
                <div class="mt-4 rounded-2xl border border-border-light dark:border-border-dark w-full p-4 lg:p-6 lg:text-lg flex items-center justify-between">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-2">
                        {#if confirmation === false}
                            <a data-sveltekit-preload-data="off" href="/meetings/{data.meeting.id}/confirmations/yes" class="b-green text-center w-16">Yes</a>
                        ...
                        {:else if confirmation === true}
                            ...
                            <a data-sveltekit-preload-data="off" href="/meetings/{data.meeting.id}/confirmations/no" class="b-red text-center w-16">No</a>
                        {:else}
                            <a data-sveltekit-preload-data="off" href="/meetings/{data.meeting.id}/confirmations/yes" class="b-green text-center w-16">Yes</a>
                            <a data-sveltekit-preload-data="off" href="/meetings/{data.meeting.id}/confirmations/no" class="b-red text-center w-16">No</a>
                        {/if}
                ...
                <div class="flex items-center justify-between mb-4 h-[34px] lg:h-[38px]">
                    ...
                    {#if data.meeting.completed == false}
                        {#if signedup}
                            ...
                            {/if}
                        {:else}
                            <button class="b-primary lg:text-lg" on:click={() => { add(data.meeting.id, warning, client ) }}>Sign Up</button>
                        {/if}
                    {/if}
                </div>
                    ...
                    {#if user != undefined}
                        <div class="flex items-center mb-3">
                            ...
                            {#if !($client == undefined || $client.permissions == undefined || !$client.permissions.includes('MODERATE_MEETINGS')) && data.meeting.completed == false}
                                <button on:click={() => { removeOtherMember(data.meeting.id, warning, user.id, client); }} class="b-default ml-auto h-[32px] lg:h-[36px]"><Icon class="text-[0.875rem] w-[0.875rem] h-[0.875rem] lg:text-[1.125rem] lg:w-[1.125rem] lg:h-[1.125rem]" scale={0} icon=remove></Icon></button>
                            {/if}
                        </div>
                    {/if}
                {:else}
            ...
            {#if $dev}
                <div class="flex items-center flex-row-reverse mt-3 opacity-50 gap-5 pr-1">
                    ...
                    <a target="_blank" href="https://console.firebase.google.com/u/0/project/frc-skywalkers{env ? "-dev" : ""}/firestore/data/~2Fteams~2F{$client?.team}~2Fmeetings~2F{data.meeting.id}" class="hover:underline">
                        Firebase
                    </a>
