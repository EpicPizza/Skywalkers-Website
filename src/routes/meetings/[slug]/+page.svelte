                ...
                {#if user != undefined}
                    <div class="flex items-center mb-3">
                        ...
                        {#if !($client == undefined || $client.permissions == undefined || !$client.permissions.includes('MODERATE_MEETINGS')) && data.meeting.completed == false}
                            <button on:click={() => { removeOtherMember(data.meeting.id, warning, user.id, client); }} class="b-default ml-3 h-[32px] lg:h-[36px]"><Icon class="text-[0.875rem] w-[0.875rem] h-[0.875rem] lg:text-[1.125rem] lg:w-[1.125rem] lg:h-[1.125rem]" scale={0} icon=remove></Icon></button>
                        {/if}
                    </div>
                {/if}
            {:else}
