...
export async function load({ cookies, request, locals, depends }) {
  ...
  if (locals.user != undefined) {
    user = {
      ...
      uid: locals.user.uid,
      pronouns: locals.firestoreUser
        ? locals.firestoreUser.pronouns
        : undefined,
  ...
  return {
    ...
    kicked: locals.kicked,
    teamInfo: locals.teamInfo,
    mode: mode,
    preload: user,
    team: locals.team,
  }; //do not use any of these directly, this layout load consistenly only runs on initial page load, instead used associated stores
}
