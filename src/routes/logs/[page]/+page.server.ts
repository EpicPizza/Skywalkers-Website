...
export async function load({ params, locals }) {
    ...
    if(!locals.firestoreUser.permissions.includes('VIEW_LOGS')) throw error(403, "Forbidden");
    ...
    const docs = (await ref.orderBy('timestamp', 'desc').offset((on - 1) * 50).limit(50).get()).docs;

    let logs = new Array<{ content: string, timestamp: Date, icon: string, id: string }>();
