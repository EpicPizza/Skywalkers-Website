<script lang="ts">
  ...
  const { enhance, form, delayed, message, allErrors, reset } = superForm(
    data.form,
  );
  ...
  async function init() {
    ...
    for (let i = 0; i < users.length; i++) {
      if (users[i].id == data.id) {
        linked =
          users[i].username +
          (users[i].discrim = "0" ? "" : "#" + users[i].discrim);
      }
    }
  }
  ...
  <Page expand size="24rem">
    ...
    {#if $verified}
      ...
      <div class="flex items-center justify-between mt-1 pb-1">
        ...
        <Menu
          raw={true}
          selected={data.preference}
          choices={["Email", "Discord", "Phone", "None"]}
          on:select={(e) => {
            changePreference(e.detail);
          }}
        >
    ...
    {:else}
      <div
        class="bg-yellow-500 bg-opacity-20 dark:bg-opacity-10 dark:text-yellow-500 text-yellow-900 flex rounded-lg p-4 gap-3"
      >
...
<Dialog bind:isOpen={confirmOpen} width="24rem">
  ...
  <p class="mt-4">
    Run the discord slash command <strong>/link</strong> on the Skywalkers Bot to
    get a code. Then enter the code here to finish linking.
  </p>

  <form use:enhance method="POST" class="mt-6">
    <div class="w-[5.375rem]">
      <p
        class="text-sm -mt-5 translate-x-2 translate-y-4 w-min whitespace-nowrap bg-white dark:bg-zinc-800 px-1 leading-4"
      >
        Code:
      </p>
      <input
        ...
        placeholder="000000"
        name="verify"
        class="p-2 mt-2 w-full bg-white border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 border-[1px] rounded-md pl-3"
        type="string"
      />
    </div>
    ...
    <div class="flex flex-row-reverse">
      <button
        disabled={$form.verify == undefined || $form.verify == ""}
        class="b-green disabled:opacity-50 disabled:cursor-not-allowed"
        >Confirm</button
      >
      <button
        class="b-default mr-2 ml-1"
    ...
    <div class="w-full">
      <form use:enhance method="POST" class="w-full">
        ...
        <div class="flex w-full flex-row-reverse">
          <button
            disabled={getID(selected) == undefined}
            class="b-green disabled:opacity-50 disabled:cursor-not-allowed"
            >Connect</button
          >
          <button
...
<Dialog bind:isOpen={phoneOpen} width="24rem">
  ...
  <div class="w-[5.375rem] mt-4">
    ...
    <input
      ...
      name="phone"
      class="p-2 mt-2 bg-white border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 border-[1px] rounded-md pl-3"
      type="string"
    />
  </div>
