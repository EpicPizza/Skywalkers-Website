<script lang="ts">
  ...
  const { form, delayed, enhance, message, allErrors } = superForm(data.form, {
    dataType: "json",
  });
    ...
    <form use:enhance method="POST">
      ...
      <div class="flex gap-1 items-center">
        ...
        <input
          ...
          name="name"
          class="w-full rounded-md p-1 px-1.5 bg-zinc-200 dark:bg-zinc-700"
        />
      </div>
      ...
      <div class="flex gap-1 mt-3 items-center">
        <DatePicker
          hideTimes
          bind:startTime={$form.ending}
          endTime={new Date()}
          bind:open={openEnd}
        />
        <p class="text-lg lg:text-xl whitespace-nowrap">Ending Date:</p>
        <p class="w-full rounded-md p-1 bg-zinc-200 dark:bg-zinc-700">
          {dnt.format($form.ending, "ddd, MMM DD")}
        </p>
        <button
          class="-ml-[1.5rem] -translate-x-[0.3rem]"
          on:click={(e) => {
        ...
        <div class="flex items-center w-full gap-1">
          <button
            on:click|preventDefault={() => {
              if ($form.length > 0) $form.length--;
            }}
            class="h-[32px] w-[32px] b-primary flex items-center justify-around"
          >
          ...
          <input
            ...
            name="name"
            type="number"
            class="w-full rounded-md p-1 px-1.5 bg-zinc-200 dark:bg-zinc-700 appearance-none"
          />
          <button
            on:click|preventDefault={() => {
              $form.length++;
      ...
      <div class="flex gap-1 mt-3 items-center">
        ...
        <RoleInput
          ...
          roles={data.roles}
          name="role"
          class="w-full rounded-md p-1 bg-zinc-200 dark:bg-zinc-700"
        />
      </div>
      <div class="flex gap-1 mt-3 items-center">
        <Tooltip text="Require one in each meeting.">
