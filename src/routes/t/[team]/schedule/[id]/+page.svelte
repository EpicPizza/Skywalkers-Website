<script lang="ts">
    ...
    unsubscribeClient = client.subscribe((current) => {
      ...
      unsubscribeFirestore = onSnapshot(ref, (snapshot) => {
        ...
        data.schedule = {
          ...
          signups: snapshot.data()?.signups as {
            [key: string]: { availability: { day: number; time: number }[] };
          },
          confirm: (snapshot.data()?.confirm ?? []) as
            | null
  ...
  //@ts-ignore
  let days = Array.apply(null, { length: data.schedule.days }).map(
    Number.call,
    Number,
  ) as number[];
  //@ts-ignore
  let times = Array.apply(null, { length: data.schedule.times }).map(
    Number.call,
    Number,
  ) as number[];
  ...
  let selected: { day: number; time: number }[] = data.schedule.signups[
    $client?.uid ?? "00000"
  ]
    ? data.schedule.signups[$client?.uid ?? "00000"].availability ?? []
    : [];
  ...
  let transfer: { day: number; time: number; mode: boolean } | undefined =
    undefined;
  ...
  function dragEnd(day: number, time: number) {
    ...
    for (let i = 0; i < days.length; i++) {
      for (let j = 0; j < times.length; j++) {
        if (
          ((days[i] >= transfer.day && days[i] <= day) ||
            (days[i] <= transfer.day && days[i] >= day)) &&
          ((times[j] >= transfer.time && times[j] <= time) ||
            (times[j] <= transfer.time && times[j] >= time))
        ) {
          let found: boolean | number = false;
  ...
  function isConfirmed(day: number, time: number) {
    ...
    for (let i = 0; i < data.schedule.confirm.length; i++) {
      for (let j = 0; j < data.schedule.confirm[i].slots.length; j++) {
        if (
          data.schedule.confirm[i].slots[j].day == day &&
          data.schedule.confirm[i].slots[j].time == time
        ) {
          return 2;
        }
      }
  ...
  async function finish() {
    ...
    const res = await fetch("/t/" + $team + "/schedule/" + $page.params.id, {
      method: "POST",
    });
...
<div
  class="border border-border-light dark:border-border-dark rounded-lg shadow-lg p-4 mb-4 bg-backgroud-light dark:bg-backgroud-dark flex items-center justify-between w-full overflow-hidden"
>
  <h1 class="text-xl overflow-hidden whitespace-nowrap overflow-ellipsis">
    {data.schedule.title}
  </h1>
  <div class="flex items-center gap-1.5">
    <button
      ...
      class="h-9 rounded-full {confirming
        ? 'b-green'
        : 'b-accent'} flex items-center gap-1.5 transition disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {#if confirming}
        Confirm
      {:else}
      ...
      {/if}
      <Icon
        scale={confirming ? "1.5rem" : "1.2rem"}
        icon={confirming ? "check" : "send"}
      ></Icon>
    </button>
  </div>
</div>
...
<div
  class="flex flex-col-reverse sm:flex-row gap-4 h-fit sm:h-[calc(100dvh-12rem)] pb-4 sm:pb-0"
>
  <div
    class="w-full overflow-x-auto border relative border-border-light dark:border-border-dark rounded-lg shadow-lg bg-backgroud-light dark:bg-backgroud-dark"
  >
    <table class="my-6 mb-8 ml-2 mr-20">
      <tr>
        ...
        {#each days as day}
          <th class="select-none pb-1">
            <p>
              {dnt.format(
                new Date(first.valueOf() + day * 1000 * 60 * 60 * 24),
                "dddd",
              )}
            </p>
            <p>
      ...
      {#each times as time}
        <tr>
          <td
            class="border-none text-right pr-2 flex flex-row-reverse min-w-[5rem]"
            ><span class="block -translate-y-3 select-none"
              >{dnt.format(
                new Date(first.valueOf() + 1000 * 60 * 60 * time),
                "h a",
              )}</span
            ></td
          >
                ...
                {#key reject}
                  <td class="p-0">
                    <button
                      ...
                      class="min-w-[6.5rem] h-full {isConfirmed(day, time) == 0
                        ? 'bg-red-500'
                        : isConfirmed(day, time) == 1
                          ? 'bg-none'
                          : reject.includes(getIndex(day, time))
                            ? 'bg-blue-500 bg-opacity-50'
                            : 'bg-green-500'} translate-y-1 transition"
                    ></button>
                  </td>
                {/key}
              {:else}
                <td class="p-0">
                  <button
                    ...
                    class="min-w-[6.5rem] h-full bg-primary-500 {isSelected(
                      day,
                      time,
                    )
                      ? isHovering(day, time)
                      ...
                      : isHovering(day, time)
                        ? 'bg-opacity-30'
                        : 'bg-opacity-0 hover:bg-opacity-10'} translate-y-1 transition"
                  ></button>
                </td>
              {/if}
            {:else}
              <td
                style="opacity: {getPercent(day, time)};"
                class="min-w-[calc(6.5rem+1px)] bg-accent-500"
              >
                {#await getPeople(day, time) then text}
                  {#if text != ""}
                    <Tooltip {text} class="w-full h-full"></Tooltip>
                  {/if}
                {/await}
              </td>
            {/if}
  ...
  <div
    bind:this={side}
    class="w-full sm:w-[50%] sm:max-w-[25rem] sm:min-w-[15rem] border border-border-light dark:border-border-dark rounded-lg shadow-lg bg-backgroud-light dark:bg-backgroud-dark p-4 overflow-auto"
  >
      ...
      {#each data.schedule.confirm as meeting}
        <div class="flex items-center">
          <p class="mt-1 mr-3">
            {dnt.format(
              new Date(
                first.valueOf() + meeting.slots[0].day * 1000 * 60 * 60 * 24,
              ),
              "MM/DD",
            )}: {dnt.format(
              new Date(
                first.valueOf() + 1000 * 60 * 60 * meeting.slots[0].time,
              ),
              "h a",
            )} - {dnt.format(
              new Date(
                first.valueOf() +
                  1000 *
                    60 *
                    60 *
                    (meeting.slots[meeting.slots.length - 1].time + 1),
              ),
              "h a",
            )}
          </p>
          {#each meeting.members as member}
            <Member id={member} let:member>
              {#await member then member}
                <Tooltip
                  class="rounded-full translate-y-0.5 w-7 h-7 border-[3px] border-backgroud-light dark:border-backgroud-dark -ml-2.5"
    ...
    {/if}
    <h2
      class="text-base opacity-50 uppercase font-bold -mb-3 {data.schedule
        .confirm != null && confirming
        ? 'mt-5'
        : ''}"
    >
      Description
    ...
    <p class="mt-1">
      {data.schedule.length} hour{data.schedule.length != 1 ? "s" : ""} each
    </p>
    <p class="mt-1">
      Min {data.schedule.member} member{data.schedule.member != 1 ? "s" : ""} per
      meeting
    </p>
        ...
        <Role id={data.schedule.group} let:role>
          ...
          {:then role}
            Group: <span
              ...
              class="w-4 h-4 -mr-0.5 block rounded-full"
            ></span>
            {role.name}
          {/await}
        </Role>
    ...
    {/if}
    <h2 class="text-base opacity-50 uppercase font-bold mt-5 mb-2">
      Signed Up
    </h2>
    <div class="flex flex-col gap-2">
      {#each Object.keys(data.schedule.signups) as key}
        ...
        <Member id={key} let:member>
          ...
          {:then member}
            <div
              class="flex items-center gap-1.5 {signup.availability.length == 0
                ? 'opacity-25'
                : 'opacity-100'}"
            >
...
<Dialog bind:isOpen={open} width="24rem" {initialFocus}>
  ...
  <form slot="content" method="POST" use:enhance>
    <div class="mt-6">
      <p>
        Schedule meetings for <span class="font-extrabold"
          >{data.schedule.title}</span
        >?
      </p>
      ...
      <div class="flex flex-row-reverse mt-6">
        ...
        <button
          ...
          class="b-red mr-2 w-16">No</button
        >
      </div>
    </div>
  </form>
