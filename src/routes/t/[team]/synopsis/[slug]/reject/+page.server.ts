...
export async function load({ params, locals, url }) {
    ...
    if(!locals.permissions.includes('MODERATE_SYNOPSES')) throw error(403, "Unauthorized.");

    if(!locals.permissions.includes('VIEW_MEETINGS')) throw error(403, "Unauthorized.");

    const db = firebaseAdmin.getFirestore();

    const ref = db.collection('teams').doc(locals.team).collection('meetings').doc(params.slug);

    const data = (await ref.get()).data();

    if(data == undefined) throw error(404, "Meeting Not Found");

    if(!locals.permissions.includes('ADD_SYNOPSES') && data.synopsis.id != locals.user.uid && data.lead.id != locals.user.uid) throw error(403, "Unauthorized.");
...
export const actions = {
    default: async function ({ locals, params }) {
        ...
        const meetingRef = db.collection('teams').doc(locals.team).collection('meetings').doc(params.slug);
        ...
        const synopsisRef = db.collection('teams').doc(locals.team).collection('synopsis').doc(params.slug);
        ...
        await db.runTransaction(async t => {
            ...
            firebaseAdmin.addLogWithTransaction("Synopsis rejected.", "summarize", uid, t, team);
        })
