<script lang="ts">
  ...
  let { tainted, form, enhance, allErrors, message, delayed, reset } =
...
>
  <div class="w-[35rem] lg:w-[44rem] overflow-hidden px-2">
    ...
    <div class="opacity-75 flex items-center mb-4 lg:mb-6">
      <Icon
        scale={0}
        class="text-[1.5rem] w-[1.5rem] h-[1.5rem] lg:text-[1.75rem] lg:w-[1.75rem] lg:h-[1.75rem] mb-auto"
        icon="info"
      ></Icon>
      <p class="ml-2 lg:text-lg">
        Editing the synopsis will not update the synopsis message sent to the
        discord server. Deleting attachments will prevent the image urls sent to
        discord from working, but images may be cached by discord for some time.
      </p>
    </div>
    <form method="POST" use:enhance>
      ...
      <PersonDialog
        ...
        ignore={idList}
        on:choosen={(event) => {
          $form.hours.push({ id: event.detail.id, time: 3 });
          $form.hours = $form.hours;
        }}
      ></PersonDialog>
