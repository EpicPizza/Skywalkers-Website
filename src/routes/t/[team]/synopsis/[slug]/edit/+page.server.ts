...
export async function load({ params, locals, url }) {
    ...
    if(!locals.permissions.includes('EDIT_SYNOPSES')) throw error(403, "Unauthorized.");

    const ref = firebaseAdmin.getFirestore().collection('teams').doc(locals.team).collection('synopsis').doc(params.slug);
...
export const actions = {
    default: async function({ request, locals, params }) {
        ...
        for(let i = 0; i < urls.length; i++) {
            for(let j = 0; j < form.data.old.length; j++) {
                if(urls[i].name + "." + urls[i].ext == form.data.old[j].name && form.data.old[j].remove) {
                    willdelete.push(`synopses/${params.slug}-${locals.team}/${urls[i].location}`);
        ...
        await db.runTransaction(async t => {
            ...
            for(let i = 0; i < form.data.hours.length; i++) {
                ...
                (() => {
                    ...
                    data.entries.push({
                        ...
                        history: [{ 
                            total: form.data.hours[i].time, 
                            link: DOMAIN + "/t/" + locals.team +  "/synopsis/" + form.data.id, reason: meeting.name + " - " + format.format(meeting.starts, "M/D/Y"), 
                            id: uid, 
                            date: new Date().valueOf(), 
            ...
            firebaseAdmin.addLogWithTransaction("Synopsis edited.", "summarize", uid, t, team);
        })
