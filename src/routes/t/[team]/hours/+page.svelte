      ...
      {#each data.hours as hours}
        <Member id={hours.member} let:member>
          {#await member then member}
            ...
            {#if role == "student"}
              <a
                href="/t/{$team}/hours/{member.id}"
                class="p-4 pr-6 w-full transition bg-black dark:bg-white bg-opacity-5 dark:bg-opacity-5 hover:bg-opacity-10 dark:hover:bg-opacity-10 flex items-center rounded-lg gap-4"
              >
                <img
