...
{#if $client == undefined}
  ...
  <p class="mt-4 text-center">
    Sign in first to continue the verification process needed to verify that you
    are a member of <Link
      href="{info?.website ?? 'https://frcskywalkers.org/'} "
      >{info?.name ?? "Skywalkers"}</Link
    >.
  </p>
  <SignIn></SignIn>
{:else if $client}
  <form method="POST" use:enhance>
    ...
    <p class="mt-4 text-center">
      Join to finish verification as <span class="font-bold underline"
        >{$client.email}</span
      >. Make sure you use the same email as the one your invitation was sent
      to.
    </p>
    <button
      class="w-full h-12 border-zinc-200 border-[1px] rounded-full mt-4 flex flex-row items-center justify-around bg-green-500 dark:border-zinc-700"
    >
