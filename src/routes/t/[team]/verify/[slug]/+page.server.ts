...
export async function load({ locals, params }) {
    if(locals.unverifiedTeam == locals.team) throw error(400, "Already part of team.");
...
export const actions = {
    default: async function({ locals, params }) {
        ...
        if(locals.firestoreUser == undefined) {
            const { found, member, team, permissions } = await findCode(params.slug, locals.user?.email, locals.unverifiedTeam);
            ...
            db.collection('users').doc(locals.user.uid).update({
