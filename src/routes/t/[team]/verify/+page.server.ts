...
export async function load(event) {
  if (event.locals.user == undefined)
    throw redirect(307, "/t/" + event.locals.unverifiedTeam);
  ...
  return {
    verify: {
      errors: {
        invalid:
          event.url.searchParams.get("invalid") === "true" ? true : false,
        redirected:
          event.url.searchParams.get("needverify") === "true" ? true : false,
      },
    },
