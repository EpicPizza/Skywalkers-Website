  ...
  <Page size="28rem">
    {#if $client == undefined}
      <h1 class="text-3xl text-center font-light">
        {info ? info.name : "Skywalkers"} Team Website
      </h1>
      <p class="mt-4 text-center">
        This is only for team members to access team resources. Our public
        website can be found at <Link
          href={info ? info.website : "https://www.frcskywalkers.org/"}
          >{info
            ? info.website
                .replaceAll("https://", "")
                .replaceAll("www.", "")
                .replaceAll("http://", "")
                .replaceAll("/", "")
            : "frcskywalkers.org"}</Link
        >.
      </p>
      <SignIn></SignIn>
    {:else if $client != undefined || "preload" in $client}
      {#if $client.teams != undefined}
        <!--At first this seems like a secruity risk, but in finished implementation this would be loaded off of load data, which would be blank for a nonteam user.-->
        <h1 class="text-3xl text-center font-light mb-6">
          {info ? info.name : "Skywalkers"} Team Website
        </h1>
        <a href="/t/{$team}/meetings" class="w-full h-16 inline-block text-xl">
          <button class="b-accent w-full h-full"> Go to Meetings </button>
        </a>
      {:else}
        <Loading></Loading>
      {/if}
