<script lang="ts">
  ...
  onMount(() => {
    ...
    unsubscribe = client
      .doc<any>(
        doc(client.getFirestore(), "teams", $team),
        structuredClone(data.codes),
      )
  ...
  {#each [...data.codes] as [key, value]}
    <div class="pr-2 w-full flex items-center justify-between mt-2">
      <button
        ...
        class="px-2 mr-2 py-1 text-left w-full transition rounded-md bg-black dark:bg-white {selected.includes(
          key,
        )
      ...
      <button
        on:click={() => {
          window.navigator.clipboard
            .writeText(
              window.location.origin + "/t/" + $team + "/verify/" + value.id,
            )
            .then(() => {
              warning.set({ color: "green", message: "Copied!" });
            })
            .catch(() => {
              warning.set({ color: "red", message: "Failed to copy." });
            });
        }}
        class="min-w-7 min-h-7 w-7 h-7 transition rounded-md dark:bg-white bg-black dark:bg-opacity-0 bg-opacity-0 dark:hover:bg-opacity-10 hover:bg-opacity-10"
      >
        <Tooltip
          text="Copy Join Link"
...
{#if data.codes.size == 0}
  <p
    class="text-red-500 text-lg dark:text-red-500 text-center w-full font-bold"
  >
...
<div class="flex items-center justify-between mt-6">
  <p class="opacity-50 text-sm">
    Select invites to delete<span class="mx-0.5">/</span>edit:
  </p>
  <div class="flex gap-2">
    <button
      on:click={handleDelete}
      disabled={selected.length == 0}
      class="b-primary disabled:opacity-50 disabled:cursor-not-allowed"
      >Delete</button
    >
  </div>
</div>
