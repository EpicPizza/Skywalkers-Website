<script lang=ts>
    ...
    async function handleSort(e: CustomEvent) {
        ...
        if(data.roles.length - from - 1 >= level || data.roles.length - to - 1 >= level) {
            return;
        }
        ...
        waiting = fetch("/t/" + $team + "/api/roles/resort", { //actually resort in database
            method: "POST",
            headers: {
    ...
    $: {
        ...
        let userLevel = $client == undefined ? undefined : !$permissions.includes("MANAGE_ROLES") ? undefined : ($currentTeam ? $currentTeam.level : 0);
