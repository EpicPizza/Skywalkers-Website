<script lang=ts>
    ...
    onMount(() => {
        ...
        unsubscribeClient = client.subscribe(async (user) => {
            ...
            unsubscribeFirestoreVerified = onSnapshot(query(collection(db, "users"), where('team', 'array-contains', $team)), (snapshot) => {
...
<Background>
    <Page size=44rem expand let:top>
        ...
        <div class="pt-4 grid grid-cols-[repeat(auto-fill,minmax(11.5rem,1fr))] gap-x-4 gap-y-1">
            {#each data.members as member}
                <div class="w-full m-2 flex items-center">
                    <a class="flex items-center overflow-hidden" href="/t/{$team}/settings/members/{member.id}">
        ...
        <div class="flex mt-8">
            <a class="b-secondary w-fit flex items-center gap-1" href="/t/{$team}/settings"><Icon scale=1.25rem icon=arrow_back></Icon>Back</a>
        </div>
    </Page>
</Background>