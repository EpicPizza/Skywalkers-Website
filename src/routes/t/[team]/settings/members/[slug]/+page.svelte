<script lang=ts>
    ...
    import type { firebaseClient, FirestoreUser, SecondaryUser } from "$lib/Firebase/firebase.js";
...
<Background>
    <Page expand size="46rem" disableLoader>
            <ProfileTag role={getRole(data.member.user)} verified id={data.member.user.id} member={data.member.user}/>
            ...
            <div class="flex w-full mt-4 gap-2 flex-row-reverse items-center">
                ...
                {/if}
                <div class="grow">
                    <a class="b-secondary w-fit flex items-center gap-1" href="/t/{$team}/settings/members"><Icon scale=1.25rem icon=arrow_back></Icon>Back</a>
                </div>
            </div>
            <OnDestroy/>
    </Page>
