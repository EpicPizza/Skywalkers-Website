...
export const POST = async ({ locals, request }) => {
  ...
  await (async () => {
    ...
    if (
      roles.length - from - 1 >= requestUserLevel ||
      roles.length - to - 1 >= requestUserLevel
    ) {
      throw error(403);
    }
  })();
  ...
  await db.runTransaction(async (transaction) => {
    ...
    for (let i = roles.length - 1; i >= 0; i--) {
      roles[i].level = level;
      transaction.update(
        db.collection("teams").doc(team).collection("roles").doc(roles[i].id),
        {
          level: roles[i].level,
        },
      );
