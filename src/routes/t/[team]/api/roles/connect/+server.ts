...
export const POST = (async ({ locals, request }) => {
    ...
    const roleRef = db.collection('teams').doc(locals.team).collection('roles').doc(role);
    ...
    await db.runTransaction(async (transaction) => {
        ...
        firebaseAdmin.addLogWithTransaction("Connected a discord role.", "group", user, transaction, team);
    })
