...
export const actions = {
  default: async ({ request, locals }) => {
    ...
    if (!locals.permissions.includes("MANAGE_ROLES") || locals.level == 0)
      throw error(403);
    ...
    if (
      (await db.collection("teams").doc(team).collection("roles").get()).docs
        .length > 250
    ) {
        ...
        users.docs.forEach(async (snapshot) => {
          ...
          for (let i = 0; i < member.teams.length; i++) {
            if (member.teams[i].team == locals.team) {
              member.teams[i].level =
                member.teams[i].level == 0 ? 0 : member.teams[i].level + 1;
            }
          }
