  ...
  <div class="w-[36rem] min-h-full relative">
    <div class="flex justify-between">
      ...
      <div class="flex gap-2 items-center">
        <svelte:element
          ...
          href="/t/{$team}/logs/1"
          class="b-primary disabled:opacity-50 rotate-180"
          ><Icon icon="double_arrow"></Icon></svelte:element
        >
        <svelte:element
          this={data.page.beginning ? "button" : "a"}
          disabled={data.page.beginning}
          href="/t/{$team}/logs/{data.page.on - 1}"
          class="b-primary disabled:opacity-50"
        ...
        <svelte:element
          this={data.page.end ? "button" : "a"}
          disabled={data.page.end}
          href="/t/{$team}/logs/{data.page.total.pages}"
          class="b-primary disabled:opacity-50"
          ><Icon icon="double_arrow"></Icon></svelte:element
        >
      </div>
    ...
    {#key data.logs}
      {#each data.logs as log}
        <div
          class="bg-accent-300 dark:bg-accent-700 w-full rounded-lg bg-opacity-25 dark:bg-opacity-25 mb-4 flex items-center px-6 gap-6"
        >
          ...
          <div class="h-full py-4 flex flex-col gap-1">
            <Member id={log.id} silent let:member>
              ...
              {:then member}
                <div class="flex items-center gap-1.5">
                  <img
                    class="w-6 opacity-75 h-6 {member.photoURL ==
                    '/unknown.webp'
                      ? 'rounded-md'
                      : 'rounded-full'}"
                    alt="Profile"
            ...
            <p class="text-lg opacity-90">
              ...
              <span class="text-sm opacity-75 ml-2"
                >({format.format(log.timestamp, "M/DD/YY h:mm:ss a")})</span
              >
            </p>
          </div>
        </div>
      {:else}
        <p class="text-center text-red-500 font-bold text-lg mb-4">
          No Logs Found
        </p>
      {/each}
    {/key}
    <div class="flex gap-2 items-center">
      ...
      <svelte:element
        ...
        ><Icon icon="arrow_back"></Icon></svelte:element
      >
      <p class="mx-2 grow text-center">
        {data.page.showing} / {data.page.total.count}
      </p>
      <svelte:element
        this={data.page.end ? "button" : "a"}
        disabled={data.page.end}
