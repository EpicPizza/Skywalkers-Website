  ...
  <div class="w-[36rem] lg:w-[44rem] min-h-full relative">
    {#if data.meeting.version != PUBLIC_MEETING_VERSION}
      <div class="flex justify-around w-full mt-8">
        <div class="flex items-center gap-2">
          ...
          <div
            class="flex-grow-[1] flex flex-col md:flex-row md:items-center md:my-0 gap-1 md:gap-0 w-full overflow-auto"
          >
            <p class="text-left lg:text-lg 4 whitespace-nowrap">
              Updating Meeting
            </p>
          </div>
        </div>
      ...
      {#if $dev}
        <div class="flex items-center w-full justify-between mt-12 opacity-50">
          <p class="flex items-center gap-1">
            {data.meeting.version}
            <Icon scale="1rem" icon="arrow_forward"></Icon>
            {PUBLIC_MEETING_VERSION}
          </p>
          <p>{data.meeting.id}</p>
        </div>
      {/if}
    {:else}
      <div class="w-full flex justify-between">
        <a
          href="/t/{$team}/meetings{data.meeting.completed
            ? '/completed'
        ...
        {#if data.meeting.completed === false}
          {#if $permissions.includes("ADD_SYNOPSES") || (data.meeting.lead != undefined && typeof data.meeting.lead == "object" ? data.meeting.lead.id == $client?.uid : false) || (data.meeting.synopsis != undefined && typeof data.meeting.synopsis == "object" ? data.meeting.synopsis.id == $client?.uid : false)}
            <a
              href="/t/{$team}/meetings/{data.meeting.id}/complete"
              class="flex gap-1 p-1 mb-2 pl-2 items-center bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 rounded-md transition lg:text-lg"
            >
              <p>Complete</p>
              <Icon
                scale={0}
        ...
        {:else}
          <a
            href="/t/{$team}/synopsis/{data.meeting.id}"
            class="flex gap-1 p-1 mb-2 pl-2 items-center bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 rounded-md transition lg:text-lg"
          >
            <p>Synopsis</p>
            <Icon
        ...
        <div class="ml-4 lg:ml-5 grow">
          ...
          {#if data.meeting.location == "Google Meet Link Insert"}
            <div class="text-lg lg:text-xl">
              On <a class="underline text-blue-900" href={data.meeting.link}
                >Google Meet</a
              >
            </div>
          {:else}
        ...
        <button
          on:click={() => {
            navigator.clipboard.writeText(
              "/meeting id:" + data.meeting.id + " team:" + $team,
            );
            warning.set({
              color: "aqua",
              message: "Discord Slash Command Copied",
            });
          }}
          class="p-2 rounded-full bg-accent-light dark:bg-accent-dark text-accent-text-light dark:text-accent-text-dark hover:brightness-95 transition"
        >
      ...
      {#if data.meeting.confirmations && signedup && !(checkConfirmation(data.meeting.confirmations, $client?.uid ?? "000000") === undefined) && !data.meeting.completed}
        {@const confirmation = checkConfirmation(
          data.meeting.confirmations,
          $client?.uid ?? "000000",
        )}
        <div
          class="mt-4 rounded-2xl border border-border-light dark:border-border-dark w-full p-4 lg:p-6 lg:text-lg flex items-center justify-between"
        >
          <div>
            <div class="flex items-center mb-4 sm:mb-2 opacity-50">
              <Icon scale="1.75rem" icon="confirmation_number"></Icon>
              <p class="text-lg lg:text-xl ml-2.5 lg:ml-3">
          ...
          <div class="flex flex-col sm:flex-row items-center gap-2">
            {#if confirmation === false}
              <button
                ...
                class="b-green text-center w-16">Yes</button
              >
              <button
                data-sveltekit-preload-data="off"
                disabled={true}
                class="b-red text-center w-16 disabled:opacity-25">No</button
              >
            {:else if confirmation === true}
              <button
                data-sveltekit-preload-data="off"
                disabled={true}
                class="b-green text-center w-16 disabled:opacity-25">Yes</button
              >
              <button
                data-sveltekit-preload-data="off"
                on:click={() => {
                ...
                class="b-red text-center w-16">No</button
              >
            {:else}
              <button
                data-sveltekit-preload-data="off"
      ...
      <div class="mt-6 flex gap-2 items-center">
        <div class="-translate-y-[1px]">
          <Icon
            class="text-[1.75rem] w-[1.75rem] h-[1.75rem] lg:text-[2.25rem] lg:w-[2.25rem] lg:h-[2.25rem]"
            scale={0}
            icon="schedule"
          ></Icon>
        </div>
        <p class="text-lg lg:text-xl">
          {format.format(data.meeting.starts, "ddd, M/D/YY: h:mm a")} - {format.format(
            data.meeting.ends,
            "h:mm a",
          )}
        </p>
      ...
      {#if data.meeting.link}
        <div class="mt-6 flex gap-2 items-center">
          ...
          ></Icon>
          <p class="text-lg lg:text-xl">
            <Link href={data.meeting.link}>{data.meeting.link}</Link>
          </p>
        </div>
      {/if}
      ...
      {#if data.meeting.role != undefined}
        <div class="mt-4 flex gap-2 items-center">
          ...
          <a
            href="/t/{$team}/settings/roles/{data.meeting.role.id}"
            class="flex items-center w-fit bg-zinc-200 dark:bg-zinc-600 rounded-full p-1 pr-4"
          >
            <div
              style="background-color: {data.meeting.role.color};"
              class="w-4 h-4 m-0.5 mx-2 lg:w-5 lg:h-5 lg:m-2 rounded-full"
            ></div>
      ...
      <div class="mt-4 flex gap-2 items-center mb-6">
        <Icon
          class="-translate-y-[1px] text-[1.75rem] w-[1.75rem] h-[1.75rem] lg:text-[2.25rem] lg:w-[2.25rem] lg:h-[2.25rem]"
          scale={0}
          icon="alarm"
        ></Icon>
        <span class="text-lg lg:text-xl">Completed:</span>
        <div
          class="{data.meeting.completed
            ? 'bg-green-500'
            : 'bg-red-500'} rounded-full h-8 w-8 lg:h-10 lg:w-10 flex justify-around items-center"
        >
          <Icon
            scale={0}
            class="text-[1.75rem] w-[1.75rem] h-[1.75rem] lg:text-[2.25rem] lg:w-[2.25rem] lg:h-[2.25rem] text-backgroud-light dark:text-backgroud-dark"
            icon={data.meeting.completed ? "check" : "close"}
          ></Icon>
        </div>
      </div>
      <div
        class="p-4 lg:p-6 pb-2 lg:pb-4 border-border-light dark:border-border-dark border-[1px] rounded-2xl"
      >
        <div
          class="flex items-center justify-between mb-4 h-[34px] lg:h-[38px]"
        >
          ...
          {#if data.meeting.completed == false && data.verified}
            {#if signedup}
              {#if !(data.meeting.role == null || (data.meeting.role && !data.meeting.role.permissions.includes("LEAVE_SIGNUP"))) || $permissions.includes("MODERATE_MEETINGS")}
                <button
                  class="b-secondary lg:text-lg"
                  on:click={() => {
                    remove(data.meeting.id, warning, client, $team);
                  }}>Leave</button
                >
              {/if}
            {:else}
              <button
                class="b-primary lg:text-lg"
                on:click={() => {
                  add(data.meeting.id, warning, client, $team);
                }}>Sign Up</button
              >
            {/if}
          {/if}
          {#if data.meeting.completed == false && data.public}
            <NewDialog
              ...
              class="b-primary lg:text-lg"
            >
              <svelte:fragment slot="button">Sign Up</svelte:fragment>
              <svelte:fragment slot="content">
                <div class="flex items-center gap-2">
                  <h1 class="text-lg mt-2">Name:</h1>
                  <input
                    bind:value={name}
                    class="rounded-md w-full mt-2 py-1 p-2"
                  />
                </div>
                <div class="flex items-center gap-2">
                ...
                <div class="flex flex-row-reverse mt-3 gap-2">
                  <button
                    on:click={() => {
                      addGuestSignup(close);
                    }}
                    class="b-primary">Add</button
                  >
                </div>
              </svelte:fragment>
            </NewDialog>
          ...
          {#if user != undefined}
            <div class="flex items-center mb-3">
              <img
                ...
                src={user.photoURL}
                class="h-8 w-8 lg:h-9 lg:w-9 rounded-full ml-1 mr-2"
              />
              <h1 class="lg:text-lg">
              ...
              {#if data.meeting.confirmations}
                ...
                {#if confirmation === null}
                  <Tooltip class="ml-1.5" text="Sent">
                    <div
                      class="w-5 h-5 my-0.5 text-backgroud-light dark:text-backgroud-dark bg-border-light dark:bg-border-dark rounded-full p-0.5"
                    >
                      <Icon scale="1rem" icon="remove"></Icon>
                    </div>
                  </Tooltip>
                {:else if confirmation === true}
                  <Tooltip class="ml-1.5" text="Confirmed">
                    <div
                      class="w-5 h-5 my-0.5 text-backgroud-light dark:text-backgroud-dark bg-green-500 rounded-full p-0.5"
                    >
                      <Icon scale="1rem" icon="check"></Icon>
                    </div>
                  </Tooltip>
                {:else if confirmation === false}
                  <Tooltip class="ml-1.5" text="Confirmed No">
                    <div
                      class="w-5 h-5 my-0.5 text-backgroud-light dark:text-backgroud-dark bg-red-500 rounded-full p-0.5"
                    >
                      <Icon scale="1rem" icon="close"></Icon>
                    </div>
                  </Tooltip>
                {/if}
              {/if}
              {#if $permissions.includes("MODERATE_MEETINGS") && data.meeting.completed == false}
                <button
                  ...
                  class="b-default ml-auto h-[32px] lg:h-[36px]"
                  ><Icon
                    class="text-[0.875rem] w-[0.875rem] h-[0.875rem] lg:text-[1.125rem] lg:w-[1.125rem] lg:h-[1.125rem]"
                    scale={0}
                    icon="remove"
                  ></Icon></button
                >
        ...
        {#each data.meeting.guests as guest}
          <div class="flex items-center mb-3">
            <div
              class="h-8 w-8 lg:h-9 lg:w-9 min-w-[2rem] lg:min-w-[2.25rem] bg-accent-500 rounded-md ml-1 mr-2 flex items-center justify-around"
            >
            ...
            <h1
              ...
              class="lg:text-lg max-w-[100vw-10rem] overflow-hidden"
            >
              {guest}
            </h1>
            <p
              class="text-sm bg-black dark:bg-white bg-opacity-10 dark:bg-opacity-10 px-1.5 py-1 rounded-md mx-2"
            >
              Guest
            </p>
            <button
              on:click={() => {
                removeGuestSignup(guest);
              }}
              class="b-default ml-auto h-[32px] lg:h-[36px]"
              ><Icon
                class="text-[0.875rem] w-[0.875rem] h-[0.875rem] lg:text-[1.125rem] lg:w-[1.125rem] lg:h-[1.125rem]"
      ...
      <div class="max-w-[calc(100vw-2rem)]">
        <div
          class="pt-4 max-w-full overflow-x-scroll flex flex-row-reverse gap-2"
        >
          {#if data.meeting.completed == false && $permissions.includes("DELETE_MEETINGS")}
            <button
              on:click={async () => {
                await deleteMeeting(data.meeting.id, client, $team);
              }}
              class="b-secondary lg:text-lg flex gap-1 items-center"
              ><Icon scale="1.25rem" icon="delete" /><span>Delete</span></button
            >
          {/if}
          {#if $permissions.includes("CREATE_MEETINGS")}
            <a
              href="/t/{$team}/meetings/{data.meeting.id}/duplicate"
              class="b-secondary lg:text-lg flex gap-1 items-center"
              ><Icon scale="1.25rem" icon="content_copy" /><span>Duplicate</span
              ></a
            >
          {/if}
          {#if data.meeting.completed == false && $permissions.includes("EDIT_MEETINGS")}
            <a
              href="/t/{$team}/meetings/{data.meeting.id}/edit"
              class="b-secondary lg:text-lg flex gap-1 items-center"
              ><Icon scale="1.25rem" icon="edit" /><span>Edit</span></a
            >
          {/if}
          <a
            href="https://www.notion.so/{data.meeting.notion.replaceAll(
              '-',
              '',
            )}"
            class="b-secondary lg:text-lg flex gap-1 items-center"
      ...
      {#if $dev}
        <div
          class="flex items-center flex-row-reverse mt-3 opacity-50 gap-5 pr-1"
        >
          ...
          <a
            ...
            href="https://console.firebase.google.com/u/0/project/frc-skywalkers{env
              ? '-dev'
              : ''}/firestore/data/~2Fteams~2F{$team}~2Fmeetings~2F{data.meeting
              .id}"
          ...
          <button
            on:click={(e) => {
              navigator.clipboard.writeText(data.meeting.id);
            }}
            class="hover:underline"
          >
            Copy ID
