<script lang=ts>
    ...
    import type { createCurrentTeam, createPermissions, Warning } from '$lib/stores.js';
    ...
    onMount(() => {
        ...
        unsubscribe = client.doc<any>(doc(client.getFirestore(), "teams", $team, "meetings", data.meeting.id), "loading").subscribe(async (value) => {
            if(value == "loading") return;

            if(value == undefined) {
                goto("/t/" + $team + "/meetings" + (data.meeting.completed ? "?completed=true&deleted=true" : "?deleted=true"));
                return;
            }
            ...
            data.meeting = {
                ...
                lead: await client.getUser(currentMeeting.lead.id) as SecondaryUser ?? getDefault(currentMeeting.lead.id),
                synopsis: currentMeeting.synopsis == null ? null : synopsis == undefined ? getDefault(currentMeeting.synopsis.id) : synopsis,
                mentor: currentMeeting.mentor == null ? null : mentor == undefined ? getDefault(currentMeeting.mentor.id) : mentor,
    ...
    async function confirm(answer: string) {
        await fetch('/t/' + $team + '/meetings/' + data.meeting.id + "/confirmations/" + answer, {
            method: 'POST',
        });
    }
    ...
    async function addGuestSignup(close: Function) {
        ...
        const res = await fetch('/t/' + $team + '/meetings/' + data.meeting.id + "?public=" + data.public, {
            method: "POST",
            headers: {
    ...
    async function removeGuestSignup(signup: string) {
        const url = new URL(PUBLIC_DOMAIN + '/t/' + $team + '/meetings/' + data.meeting.id + "?public=" + data.public);
        ...
        {:else}
            ...
            <div class="p-4 lg:p-6 pb-2 lg:pb-4 border-border-light dark:border-border-dark border-[1px] rounded-2xl">
                ...
                {#each data.meeting.guests as guest}
                    <div class="flex items-center mb-3">
                        <div class="h-8 w-8 lg:h-9 lg:w-9 min-w-[2rem] lg:min-w-[2.25rem] bg-accent-500 rounded-md ml-1 mr-2 flex items-center justify-around">
