...
export async function load({ params, locals }) {
  ...
  if (!locals.permissions.includes("EDIT_MEETINGS"))
...
export const actions = {
  default: async ({ request, locals, params, url }) => {
    if (locals.user == undefined) {
      throw error(403, "Sign In Required");
    }
    ...
    const validated = await validateFormMeeting(
      form,
      locals.user.uid,
      locals.team,
    );
    ...
    if (validated === 0) {
      try {
        await editMeetingFromForm(
          await getMeeting(params.slug, locals.user.uid, locals.team),
          form,
          locals.user.uid,
          locals.team,
        );
      ...
      if (url.searchParams.get("redirect") == "completed") {
        ...
        throw redirect(
          307,
          "/t/" + locals.team + "/meetings/" + params.slug + "?edited=true",
        );
      }
    } else {
      return validated;
