...
<svelte:head>
    <title>{info?.name ?? "Skywalkers"} | Edit Meeting</title>
</svelte:head>
...
<div class="min-h-[calc(100dvh)] p-8 pt-[88px] flex justify-around">
    <div class="w-[36rem] lg:w-[44rem]">
        ...
        <h1 class="text-2xl lg:text-3xl pb-4">Edit Meeting:</h1>
        <form method="POST" use:enhance>
            ...
            <div class="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]">
                ...
                <input name=location bind:value={$form.location} class="lg:p-1 lg:pl-2 lg:text-lg w-full rounded-md p-1 bg-zinc-200 dark:bg-zinc-700 {$form.location == "Google Meet Link Insert" ? "italic text-opacity-80" : ""}"/>
                <button class="-ml-[1.5rem] -translate-x-[0.3rem]" on:click={(e) => { e.preventDefault(); $form.location = 'Google Meet Link Insert'; $form.virtual = true; }}>
