<script lang="ts">
  ...
  export const snapshot = {
    capture: middleManCapture,
    restore: middleManRestore,
  };
...
<svelte:head>
  <title>{info?.name ?? "Skywalkers"} | Complete Meeting</title>
</svelte:head>

<div
  class="min-h-[calc(100dvh)] p-4 sm:p-8 pt-[88px] sm:pt-[88px] flex justify-around"
>
  <div class="w-[35rem] lg:w-[44rem] overflow-hidden px-2">
    ...
    <div
      class="p-4 border-border-light dark:border-border-dark border-[1px] rounded-3xl"
    >
      <div
      ...
      >
        ...
        <div class="ml-4 lg:ml-5 grow">
          ...
          <div class="text-lg lg:text-xl opacity-80 whitespace-nowrap">
            At {data.meeting.location}
          </div>
        </div>
        <a
          href="/meetings/{data.meeting.id}/edit?redirect=completed"
          class="mb-auto"
        >
          <button
            on:click={() => {
      ...
      <div class="mt-6 flex gap-2 items-center">
        ...
        <p class="text-lg lg:text-xl">
          {format.format(data.meeting.starts, "M/D/YY, h:mm a")} - {format.format(
            data.meeting.ends,
            "h:mm a",
          )}
        </p>
      ...
      {#if data.meeting.role != undefined}
        <div class="mt-4 flex gap-2 items-center">
          ...
          <a
            href="/settings/roles/{data.meeting.role.id}"
    ...
    <form enctype="multipart/form-data" method="POST" use:enhance>
      <div class="mt-8 items-center w-full">
        <div class="flex justify-between">
          ...
          <p
            class="lg:text-lg mt-auto mb-3 {$form.synopsis.length > 10000
              ? 'text-red-500 dark:text-red-500 font-bold'
              : 'opacity-50'}"
          >
            {$form.synopsis.length}/10000
          </p>
        </div>
        <textarea
          ...
          class="p-5 lg:text-lg w-full rounded-xl bg-zinc-200 dark:bg-zinc-700 h-52"
        />
      </div>
      ...
      <div
        class="border-border-light dark:border-border-dark border-[1px] rounded-xl p-4 flex flex-col gap-4"
      >
            ...
            {:then user}
              ...
              <div class="flex items-center gap-2.5">
                <img
                  ...
                  alt="{user.displayName}{user.pronouns == ''
                    ? ''
                    : ' (' + user.pronouns + ')'}'s Profile"
                  src={user.photoURL}
                />
                <p
                  class="text-lg lg:text-xl grow overflow-hidden whitespace-nowrap overflow-ellipsis"
                >
                ...
                <div class="flex gap-1.5">
                  {#if role == "student"}
                    <button
                      disabled={member.time <= 0}
                      class="b-primary disabled:cursor-not-allowed w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-around"
                      on:click|preventDefault={() => {
                        if (member.time < 0.25) {
                          member.time = 0;
                        } else if (member.time > 12) {
                          member.time = 12;
                        } else {
                          member.time -= 0.25;
                        }
                      }}><Icon scale="1.25rem" icon="remove"></Icon></button
                    >
                    <input
                      step="0.25"
                      class="w-16 p-2 py-0.5 text-lg lg:text-xl text-center rounded-md"
                      type="number"
                      bind:value={member.time}
                    />
                    <button
                      disabled={member.time >= 12}
                      class="b-primary disabled:cursor-not-allowed w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-around"
                      on:click|preventDefault={() => {
                        if (member.time > 11.75) {
                          member.time = 12;
                        } else if (member.time < 0) {
                          member.time = 0;
                        } else {
                          member.time += 0.25;
                        }
                      }}><Icon scale="1.25rem" icon="add"></Icon></button
                    >
                  {/if}
                  <button
                    class="b-accent w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-around"
                    on:click|preventDefault={() => {
                      $form.hours.splice(i, 1);
        ...
        <div class="flex items-center">
          ...
          <button
            class="b-primary whitespace-nowrap lg:p-1 lg:px-2 lg:text-lg"
            on:click|preventDefault={() => {
              open = !open;
            }}>Add Member</button
          >
      ...
      <div class="flex gap-2 mt-6 items-center lg:gap-3">
        <button
          on:click|preventDefault={() => {
            $form.discord = !$form.discord;
          }}
          class="p-1 {$form.discord
      ...
      <PersonDialog
        ...
        ignore={idList}
        on:choosen={(event) => {
          for (let i = 0; i < event.detail.id.length; i++) {
            $form.hours.push({ id: event.detail.id[i], time: data.length });
          }
          $form.hours = $form.hours;
        }}
      ></PersonDialog>
      ...
      <div class="flex items-center mt-7 gap-4">
        ...
        >
          ...
          <Icon
            class="text-[1.25rem] w-[1.25rem] h-[1.25rem] lg:text-[1.75rem] lg:w-[1.75rem] lg:h-[1.75rem]"
            scale={0}
            icon="check"
          ></Icon>
        </button>
