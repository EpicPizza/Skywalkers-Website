...
<div class="min-h-[calc(100dvh)] p-4 sm:p-8 pt-[88px] sm:pt-[88px] flex justify-around">
    <div class="w-[35rem] lg:w-[44rem] overflow-hidden px-2">
        ...
        <form enctype=multipart/form-data method=POST use:enhance>
            ...
            <PersonDialog multiple bind:open ignore={idList} on:choosen={(event) => { for(let i = 0; i < event.detail.id.length; i++) { $form.hours.push({ id: event.detail.id[i], time: data.length }); } $form.hours = $form.hours; }}></PersonDialog>
