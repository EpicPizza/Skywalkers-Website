...
import { FieldValue, type DocumentReference, Timestamp } from 'firebase-admin/firestore';
...
export async function load({ params, locals, url }) {
    ...
    try {
        meeting = await getFetchedMeeting(params.slug, locals.user.uid, locals.team);
    } catch(e: any) {
    ...
    if(!(locals.permissions.includes('ADD_SYNOPSES') || locals.user.uid == meeting.lead.id || (meeting.synopsis && locals.user.uid == meeting.synopsis.id))) throw error(403, "Unauthorized.");
...
export const actions = {
    default: async function({ request, locals, params }) {
        ...
        for(let i = 0; i < queues.length; i++) {     
            ...
            const desRef = firebaseAdmin.getBucket().file(`synopses/${params.slug}-${locals.team}/${code}.${queues[i].type?.ext ?? "txt"}`);
            ...
            try {
                ...
                await desRef.setMetadata({ contentDispoition: 'inline; filename*=utf-8\'\'"' + queues[i].name + '.' + queues[i].type?.ext ?? "txt" + '"', "Content-Type": queues[i].type?.mime ?? "text/plain" });
               //await locFol.delete();
            } catch(e) {
                console.log(e);
        ...
        await db.runTransaction(async t => {
            ...
            for(let i = 0; i < form.data.hours.length; i++) {
                ...
                toUpdate.push({ref, data: {
                    ...
                    entries: FieldValue.arrayUnion({ total: form.data.hours[i].time, id: form.data.id, latest: 0, history: [{ total: form.data.hours[i].time, link: DOMAIN + "/t/" + locals.team + "/synopsis/" + form.data.id, reason: meeting.name + " - " + format.format(meeting.starts, "M/D/Y"), id: uid, date: new Date().valueOf(), indicator: meetingIndicator }]})
                }});
            }
            ...
            await completeMeeting(t, team, notion, meetingRef, true, synopsis);

            firebaseAdmin.addLogWithTransaction("Meeting completed.", "event", uid, t, team);
        ...
        throw redirect(307, "/t/" + locals.team + "/meetings/" + form.data.id + "");
    }
}