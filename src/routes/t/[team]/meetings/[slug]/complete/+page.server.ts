...
import {
  ...
  type Hours,
  type Indicator,
} from "$lib/Hours/hours.server.js";
...
import {
  ...
  getQueueById,
  type Queue,
} from "$lib/Upload/helpers.server.js";
...
export async function load({ params, locals, url }) {
  ...
  if (
    !(
      ...
      locals.user.uid == meeting.lead.id ||
      (meeting.synopsis && locals.user.uid == meeting.synopsis.id)
    )
  )
  ...
  default: async function ({ request, locals, params }) {
    ...
    for (let i = 0; i < queues.length; i++) {
      ...
      try {
        ...
        await desRef.setMetadata({
          ...
          "Content-Type": queues[i].type?.mime ?? "text/plain",
        });
        //await locFol.delete();
      } catch (e) {
        console.log(e);
    ...
    await db.runTransaction(async (t) => {
      ...
      let toUpdate: {
        ...
        data: { total: number; entries: FieldValue };
      }[] = new Array();
    ...
    if (form.data.discord) {
      ...
      await sendSynopsis(
        ...
        (meeting.role && meeting.role.connectTo
          ? "<@&" + meeting.role.connectTo + "> "
          : "") +
          `<t:${meeting.starts.valueOf() / 1000}:F> to <t:${meeting.ends.valueOf() / 1000}:t>` +
          (nameString.length != 0 ? " ( " + nameString + " )" : "") +
          ": " +
          form.data.synopsis,
        urls,
        DOMAIN + "/t/" + locals.team + "/synopsis/" + form.data.id,
