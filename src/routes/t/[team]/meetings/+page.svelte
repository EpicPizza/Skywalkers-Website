<script lang="ts">
  ...
  let unsubscribeBottom: Unsubscriber | undefined = bottom.subscribe(
    (isBottom) => {
      if (!browser) return;
  ...
  onMount(() => {
    ...
    unsubscribeClient = client.subscribe((user) => {
      ...
      const ref = query(
        ...
        orderBy("starts"),
        limit(number),
        ...(schedule != null ? [where("schedule", "==", schedule)] : []),
      );
    ...
    unsubscribeFirestore = onSnapshot(ref, async (snapshot) => {
      ...
      for (let i = 0; i < firestoreMeetings.length; i++) {
        if (firestoreMeetings[i].data().version != PUBLIC_MEETING_VERSION) {
          fetch(
            "/t/" + $team + "/meetings/" + firestoreMeetings[i].id + "/update",
          );
        }
        ...
        meetings.push({
          ...
          lead:
            (await client.getUser(firestoreMeetings[i].data().lead.id)) ??
            getDefault(firestoreMeetings[i].data().lead.id),
          synopsis:
            firestoreMeetings[i].data().synopsis == null
              ? getDefault("000000")
              : (await client.getUser(
                  firestoreMeetings[i].data().synopsis.id,
                )) ?? getDefault(firestoreMeetings[i].data().synopsis.id),
          mentor:
            firestoreMeetings[i].data().mentor == null
              ? getDefault("000000")
              : (await client.getUser(firestoreMeetings[i].data().mentor.id)) ??
                getDefault(firestoreMeetings[i].data().mentor.id),
          ...
          role:
            firestoreMeetings[i].data().role != null
              ? (await client.getRole(
                  firestoreMeetings[i].data().role,
                  $team,
                )) ?? null
              : null,
          signups: await Promise.all(
            Array.from(firestoreMeetings[i].data().signups, async (signup) => {
              return (
                (await client.getUser(signup as string)) ?? getDefault("000000")
              );
            }),
          ),
              ...
              <MenuItems>
                ...
                >
                  ...
                  {#if $permissions.includes("DELETE_MEETINGS") || $permissions.includes("CREATE_MEETINGS") || $permissions.includes("LEAVE_SIGNUP")}
                    ...
                    >
                      {#if $selected.includes(meeting.id)}
                        Deselect
                        <Icon scale="1.15rem" icon="radio_button_checked"
                        ></Icon>
                      {:else}
                        Select
                  ...
                  {#if $permissions.includes("EDIT_MEETINGS")}
                    <MenuItem
                      href="/t/{$team}/meetings/{meeting.id}/edit"
                      class="flex items-center justify-between float-left px-2 py-1 bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 transition w-full text-left rounded-md"
                    >
                      Edit
                  ...
                  {#if $permissions.includes("ADD_SYNOPSES") || meeting.lead.id == $client?.uid || (meeting.synopsis && meeting.synopsis.id == $client?.uid)}
                    <MenuItem
                      href="/t/{$team}/meetings/{meeting.id}/complete"
                      class="flex items-center justify-between float-left px-2 py-1 bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 transition w-full text-left rounded-md"
                    >
                      Complete
...
{:else if data.verified}
  ...
  >
    <div class="flex px-1.5 gap-1.5 h-full">
      {#if $permissions.includes("DELETE_MEETINGS")}
        <button
          on:click={selected.actions.delete}
          class="w-full min-w-[150px] text-center lg:text-lg my-1.5 rounded-md bg-red-500 dark:bg-red-500 bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 transition flex justify-around items-center"
        >
      ...
      {#if $permissions.includes("LEAVE_SIGNUP")}
        <button
          on:click={selected.actions.signup}
          class="w-full min-w-[150px] text-center lg:text-lg my-1.5 rounded-md bg-green-500 dark:bg-green-500 bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 transition flex justify-around items-center"
        >
        ...
        <button
          on:click={selected.actions.leave}
          class="w-full min-w-[150px] text-center lg:text-lg my-1.5 rounded-md bg-orange-500 dark:bg-orange-500 bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 transition flex justify-around items-center"
        >
