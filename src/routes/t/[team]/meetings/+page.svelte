<script lang=ts>
    ...
    function loadNext() {
        ...
        unsubscribeClient = client.subscribe((user) => {
            try {
                ...
                if(order == 'Upcoming') {
                    const ref = query(collection(db, "teams", $team, "meetings"), where("completed", "==", data.completed), where("starts", ">=", today), orderBy("starts"), limit(number));
                    startListener(ref);
                } else {
                    const ref = query(collection(db, "teams", $team, "meetings"), where("completed", "==", data.completed), where("starts", "<", today), orderBy("starts", "desc"), limit(number));
                    startListener(ref);
                }
            } catch(e) {
                console.error(e);
    ...
    onMount(() => {
        ...
        unsubscribeClient = client.subscribe((user) => {
            ...
            const ref = query(collection(db, "teams", $team, "meetings"), where("completed", "==", data.completed), where("starts", ">=", today), orderBy("starts"), limit(number), ...(schedule != null ? [ where("schedule", "==", schedule) ] : []));
        ...
        unsubscribeFirestore = onSnapshot(ref, async (snapshot) => {
            ...
            for(let i = 0; i < firestoreMeetings.length; i++) {
                if(firestoreMeetings[i].data().version != PUBLIC_MEETING_VERSION) {
                    fetch('/t/' + $team + '/meetings/' + firestoreMeetings[i].id + "/update");
                }
                ...
                meetings.push({
                    ...
                    role: firestoreMeetings[i].data().role != null ? await client.getRole(firestoreMeetings[i].data().role, $team) ?? null : null,
                    ...
                    guests: firestoreMeetings[i].data().guests,
                });
            }
    ...
    function changeOrder(newOrder: "Upcoming" | "Recent") {
        ...
        unsubscribeClient = client.subscribe((user) => {
            ...
            if(newOrder == 'Upcoming') {
                ...
                const ref = query(collection(db, "teams", $team, "meetings"), where("completed", "==", data.completed), where("starts", "<", today), orderBy("starts", "desc"), limit(number), ...(schedule != null ? [ where("schedule", "==", schedule) ] : []));
                startListener(ref);
            }
        })
    }
    ...
    function Select() {
        ...
        const actions = {
            leave: async () => {
                ...
                for(let i = 0; i < n.length; i++) {
                    await remove(n[i], warning, client, $team);
                }
            ...
            signup: async () => {
                ...
                for(let i = 0; i < n.length; i++) {
                    try {
                        await add(n[i], warning, client, $team);
                    } catch(e) {
                        console.log(e);
                    }
                }
            ...
            delete: async () => {
                ...
                await deleteMeetings(n, client, $team);
    ...
    function gotoMeeting(id: string) {
        if($selected.length != 0) {
            ...
            leave = "/t/" + $team +  "/meetings/" + id + (data.public ? "?public=" + data.public : "");
        } else {
            goto("/t/" + $team +  "/meetings/" + id + (data.public ? "?public=" + data.public : ""));
        }
    }
...
<svelte:head>
    <title>{info?.name ?? "Skywalkers"} | {data.completed ? "Completed" : "Active"} Meetings</title>
</svelte:head>
...
<div class="min-h-[calc(100dvh-3rem)] lg:min-h-[calc(100dvh-3.5rem)] pt-[4.5rem] w-full bg-zinc-100 dark:bg-zinc-900 { $permissions.includes('CREATE_MEETINGS') ? "pb-[4.5rem] lg:pb-16" : ""} overflow-x-auto">
    ...
    <div class="p-4 pb-2">
                    ...
                    <Menu on:click={(e) => { e.stopPropagation(); }}>
                        ...
                        <MenuItems>
                            <div style="max-height: {maxheight}px;" on:introstart={menuCheck} transition:slide={{ duration: 0, }} bind:this={menu} class="absolute z-10 right-6 max-w-[9.5rem] bg-backgroud-light dark:bg-backgroud-dark p-1.5 border-border-light dark:border-border-dark border-[1px] rounded-lg shadow-lg shadow-shadow-light dark:shadow-shadow-dark overflow-auto">
                                ...
                                {#if (!(meeting.role == null || (meeting.role && !meeting.role.permissions.includes('LEAVE_SIGNUP'))) || $permissions.includes('MODERATE_MEETINGS')) && data.verified}
                                    <MenuItem on:click={async (event) => { 
                                        ...
                                        if(includesSignup(meeting.signups)) {
                                            ...
                                            await add(meeting.id, warning, client, $team);
                                        }
                                ...
                                {/if}
                                {#if $permissions.includes('EDIT_MEETINGS')}
                                    <MenuItem href="/t/{$team}/meetings/{meeting.id}/edit" class="flex items-center justify-between float-left px-2 py-1 bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 transition w-full text-left rounded-md">
                                        Edit
                                ...
                                {/if}
                                {#if $permissions.includes('DELETE_MEETINGS')}
                                    <MenuItem on:click={async (event) => {
                                        event.preventDefault(); 
                                        event.stopPropagation(); 

                                        await deleteMeeting(meeting.id, client, $team);
                                    }} class="flex items-center justify-between float-left px-2 py-1 bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 transition w-full text-left rounded-md">
                                        Delete      
                                        <Icon scale=1.15rem icon=delete></Icon>                     
                                    </MenuItem>
                                {/if}
                                {#if $permissions.includes('ADD_SYNOPSES') || meeting.lead.id == $client?.uid || (meeting.synopsis && meeting.synopsis.id == $client?.uid)}
                                    <MenuItem href="/t/{$team}/meetings/{meeting.id}/complete" class="flex items-center justify-between float-left px-2 py-1 bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 transition w-full text-left rounded-md">
                                        Complete
        ...
        {:else}
            ...
            {/if}
        {/each}
        {#if data.meetings.length != 0 && !reachedEnd && data.verified}
...
{#if $selected.length == 0 && data.verified}
    <div transition:slide|local class="h-12 lg:h-14 sticky z-[5] bottom-0 border-border-light dark:border-border-dark border-t-[1px] bg-backgroud-light dark:bg-backgroud-dark w-full flex px-1.5 gap-1.5">
        <svelte:element this={data.completed ? "a" : "div"} href={data.completed ? "/t/" + $team + "/meetings" : undefined} class="w-full text-center lg:text-lg my-1.5 rounded-md {data.completed ? "bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10" : "bg-accent-500 text-black text-dcursor-not-allowed"} transition flex justify-around items-center">
            Active
        </svelte:element>
        <svelte:element this={data.completed ? "div" : "a"} href={data.completed ? undefined : "/t/" + $team + "/meetings/completed/1"} class="w-full lg:text-lg  text-center my-1.5 rounded-md {!data.completed ? "bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10" : "bg-accent-500 text-black text-dcursor-not-allowed"} transition flex justify-around items-center">
            Completed
        </svelte:element>
    </div>  
{:else if data.verified}
    <div on:outroend={() => { checkLeave(); }} transition:slide|local class="h-12 lg:h-14 sticky z-[5] bottom-0 border-border-light dark:border-border-dark border-t-[1px] bg-backgroud-light dark:bg-backgroud-dark overflow-auto">
        <div class="flex px-1.5 gap-1.5 h-full">
            ...
            {/if}
            {#if $permissions.includes('LEAVE_SIGNUP')}
