    ...
    <div use:melt={$content("tab-1")}>
      ...
      <form method="POST" action="?/onetime" use:onetimeEnhance>
        ...
        <div class="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]">
          ...
          <input
            name="location"
            bind:value={$onetimeForm.location}
            class="lg:p-1 lg:pl-2 lg:text-lg w-full rounded-md p-1 bg-zinc-200 dark:bg-zinc-700 {$onetimeForm.location ==
          ...
          <button
            ...
            on:click={(e) => {
              e.preventDefault();
              $onetimeForm.location = "Google Meet Link Insert";
              $onetimeForm.virtual = true;
            }}
          >
        ...
        <div class="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]">
          <DatePicker
            bind:startTime={$onetimeForm.starts}
            bind:endTime={$onetimeForm.ends}
            bind:open
          />
          ...
          <p
            class="lg:p-1 lg:pl-2 lg:text-lg w-full rounded-md p-1 bg-zinc-200 dark:bg-zinc-700"
          >
            {format.format($onetimeForm.starts, "ddd, MMM DD")}: {format.format(
              $onetimeForm.starts,
              "h:mm a",
            )} - {format.format($onetimeForm.ends, "h:mm a")}
          </p>
          <button
            class="-ml-[1.5rem] -translate-x-[0.3rem]"
            on:click={(e) => {
        ...
        <div class="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]">
          ...
          <IconChooser
            name="thumbnail"
            bind:value={$onetimeForm.thumbnail}
        ...
        <div class="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]">
          ...
          <PersonChooser
            optional
            name="synopsis"
            bind:value={$onetimeForm.synopsis}
            class="lg:p-1 lg:pl-2 lg:text-lg w-full rounded-md p-1 bg-zinc-200 dark:bg-zinc-700 text-left"
          ></PersonChooser>
        </div>
        <div class="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]">
        ...
        <input
          ...
          name="virtual"
          type="checkbox"
          bind:checked={$onetimeForm.virtual}
        />
        <div class="flex gap-2 mt-6 items-center lg:gap-3">
          <button
            on:click|preventDefault={() => {
              $onetimeForm.virtual = !$onetimeForm.virtual;
            }}
            class="p-1 {$onetimeForm.virtual
    ...
    <div use:melt={$content("tab-2")}>
      ...
      <form action="?/recurring" method="POST">
        ...
        <div class="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]">
          ...
          <input
            name="location"
            bind:value={$recurringForm.location}
            class="lg:p-1 lg:pl-2 lg:text-lg w-full rounded-md p-1 bg-zinc-200 dark:bg-zinc-700 {$recurringForm.location ==
            'Google Meet Link Insert'
              ? 'italic text-opacity-80'
              : ''}"
          />
          <button
            ...
            on:click={(e) => {
              e.preventDefault();
              $recurringForm.location = "Google Meet Link Insert";
              $recurringForm.virtual = true;
            }}
          >
            <Icon scale="1.25rem" icon="videocam"></Icon>
          </button>
        ...
        <div class="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]">
          ...
          <IconChooser
            name="thumbnail"
            bind:value={$recurringForm.thumbnail}
            class="lg:p-1 lg:pl-2 lg:text-lg w-full rounded-md p-1 bg-zinc-200 dark:bg-zinc-700 text-left"
          ></IconChooser>
        </div>
        <div class="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]">
        ...
        <div class="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]">
          ...
          <TimePicker
            bind:start={$recurringForm.starts}
            bind:end={$recurringForm.ends}
          ></TimePicker>
        </div>
        <Combobox
          bind:value={$recurringForm.recurring}
          list={recurringList}
          labelString="Recurring:"
          placeholder=""
          className="flex gap-1 mt-4 items-center min-w-[350px] max-w-[75%]"
        ...
        {#if $recurringForm.recurring == "Monthly"}
          <div transition:slide class="pt-4">
            <Combobox
              bind:value={$recurringForm.week}
              list={weekList}
              labelString="Week:"
              placeholder="First Week of the Month"
              className="flex gap-1 items-center min-w-[350px] max-w-[75%]"
              labelClass="text-lg lg:text-xl"
              inputClass="lg:p-1 lg:pl-2 lg:text-lg w-full rounded-md p-1 bg-zinc-200 dark:bg-zinc-700"
              let:detail
              let:selected
        ...
        <div class="flex gap-2 mt-6 items-center lg:gap-3">
          <button
            on:click|preventDefault={() => {
              $recurringForm.virtual = !$recurringForm.virtual;
            }}
            class="p-1 {$recurringForm.virtual
              ? 'border-blue-700 dark:border-blue-500 bg-blue-200 dark:bg-blue-900 text-blue-700 dark:text-blue-500'
              : 'border-border-light dark:border-border-dark text-border-light dark:text-border-dark'} border-[1px] rounded-md m-0.5 transition disabled:cursor-not-allowed disabled:opacity-40"
          >
            <Icon icon="check"></Icon>
