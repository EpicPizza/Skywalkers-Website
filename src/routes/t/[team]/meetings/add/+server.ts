...
export const PATCH = async ({ locals, request }) => {
  ...
  if (!locals.permissions.includes("CREATE_MEETINGS"))
  ...
  const colRef = db.collection("teams").doc(locals.team).collection("meetings");
  ...
  for (let i = 0; i < meetings.length; i++) {
    ...
    if (meetings[i].starts.valueOf() < today.valueOf())
      return json("Past meetings cannot be made.");
    ...
    if (meetings[i].ends.valueOf() <= meetings[i].starts.valueOf())
      return json("Start time must be before end time.");
