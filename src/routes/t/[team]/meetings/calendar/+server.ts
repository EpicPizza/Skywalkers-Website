...
export async function POST({ request, params, url, locals }) {
    ...
    const ref = db.collection('teams').doc(team).collection('meetings').where('update', '==', true).limit(100);
    ...
    try {
        for(let i = 0; i < docs.length; i++) {
            ...
            const users = await firebaseAdmin.getAuth().getUsers(Array.from(signups, (x) => ({ uid: x as string })));
