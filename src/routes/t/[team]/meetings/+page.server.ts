...
export async function load({ locals, url }) {
    const isPublic = (await firebaseAdmin.getFirestore().collection('teams').doc(locals.unverifiedTeam ?? "000000").collection('settings').doc('meetings').get()).data()?.public ?? false;

    let original = locals.team;

    if(isPublic && safeCompare(isPublic, url.searchParams.get('public') ?? "000000")) {
    ...
    const ref = firebaseAdmin.getFirestore().collection('teams').doc(locals.team).collection('meetings').where('completed', '==', false).where('starts', '>=', today).orderBy('starts').limit(50);

    const roles = await getRolesAsCache(locals.team);
    const members = await getMemberCache(locals.team, roles);
    const meetings = await getFetchedMeetingsOptimized(ref, locals.user ? locals.user.uid : "000000", locals.team, members, roles);

    return { 
        meetings: meetings as unknown as FetchedMeeting[], 
        reachedEnd: (await firebaseAdmin.getFirestore().collection('teams').doc(locals.team).collection('meetings').where('completed', '==', false).where('starts', '>=', today).count().get()).data().count == meetings.length, 
        meetingsShown: meetings.length, 
        loading: false, 
        completed: false, 
        deleted: url.searchParams.get("deleted") === 'true' ? true : false, 
        verified: original != null,
        public: isPublic && safeCompare(isPublic, url.searchParams.get('public') ?? "000000") ? url.searchParams.get('public') ?? "000000" : null,
    };
}