<script lang="ts">
    ...
    $: {
        ...
        for(let i = 0; i < data.hours.entries.length; i++) {
            ...
            let day = date.valueOf() - date.getMilliseconds() - (date.getSeconds() * 1000) - (date.getMinutes() * 60 * 1000) - (date.getHours() * 60 * 60 * 1000);
            ...
            if(i > 0) {
                ...
                let lastDay = lastDate.valueOf() - lastDate.getMilliseconds() - (lastDate.getSeconds() * 1000) - (lastDate.getMinutes() * 60 * 1000) - (lastDate.getHours() * 60 * 60 * 1000);
        ...
        console.log(brushedRawData);

        brushedData = brushedRawData.slice((brushExtents[0] || 0) * brushedRawData.length, (brushExtents[1] || 1) * brushedRawData.length);
    }
    ...
    function showTime(hours: typeof data["hours"]["entries"], index: number) {
        ...
        return currentTime.getDate() != previousTime.getDate() || currentTime.getMonth() != previousTime.getMonth() || currentTime.getFullYear() != previousTime.getFullYear();
    }
</script>
        ...
        <Page size="24rem" expand>
            ...
            >
                <p class="text-xl font-extrabold">
                    Total: {data.hours.total} hour{data.hours.total == 1
                        ? ""
                        : "s"}
                </p>
            </div>
            ...
            {#if data.hours.entries.length > 0}
                <div class="w-full h-40 mb-6 relative pl-6">
                    ...
                    <p class="text-sm font-bold absolute left-0 top-1/2 -rotate-90 -translate-y-1/2 -translate-x-3">Hours</p>
                </div>
            {:else} 
            ...
            {#each [...data.hours.entries].reverse() as entry, i (entry.id)}
                ...
                >
                    <Icon
                        scale="1.75rem"
                        style="color: {entry.history[entry.latest].indicator
                            .color}"
                        icon={entry.history[entry.latest].indicator.icon}
                    />
                    <div
                        class="grow w-full overflow-hidden flex gap-3 items-center"
                    >
                        ...
                        >
                            {entry.history[entry.latest].reason ??
                                "No Reason Given"}
                        </svelte:element>
                    </div>
                    <span
