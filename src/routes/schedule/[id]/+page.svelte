<script lang=ts>
    ...
    onMount(() => {
        ...
        unsubscribeClient = client.subscribe((current) => {
            if(current == undefined || current.team == undefined || 'preload' in current) return;

            let database = client.getFirestore();

            let ref = doc(database, "teams", current.team, "schedule", $page.params.id);
    ...
    async function finish() {
        ...
        if(res.status == 406) {
            ...
            await goto("/meetings?schedule=" + $page.params.id);
        }
    }

    $: {
        if(!($client == undefined || $client.team == undefined || 'preload' in $client)) {
            let database = client.getFirestore();

            let ref = doc(database, "teams", $client?.team ?? "000000", "schedule", $page.params.id);
...
<svelte:head>
    <title>Skywalkers | Scheduler</title>
</svelte:head>
                                ...
                                {#key reject}
                                    <td class="p-0">
                                        <button on:click={() => { 
                                            ...
                                            console.log(reject.includes(getIndex(day, time)));
