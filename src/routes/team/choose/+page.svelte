  ...
  <Page expand size="20rem">
    ...
    {#each data.teams as team}
      <a href="/t/{team.id}" class="w-full inline-block mt-1">
        <button
          on:click={() => {
            $teamInfo.set(team.id, team);
          }}
          class="bg-black dark:bg-white bg-opacity-10 dark:bg-opacity-10 transition p-2 hover:bg-opacity-20 dark:hover:bg-opacity-20 rounded-md w-full h-full flex justify-around"
        >
