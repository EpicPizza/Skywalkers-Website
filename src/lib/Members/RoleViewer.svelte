...
<div class="flex mt-6 gap-6 sm:gap-8 flex-col sm:flex-row">
    <div class="w-full flex-col flex">
        <div class="p-4 pb-14 w-full h-full border-[1px] border-border-light dark:border-border-dark rounded-xl relative">
            <p class="mb-2">Roles:</p>

            {#if loading || deleting != ''}
                <div class="absolute top-4 right-4">
                    <div class="scale-75 flex items-center w-[2.125rem] h-[2.125rem] overflow-hidden">
                        <Loading></Loading>
                    </div>
                </div>
            {/if}
...
<Dialog isOpen={open} width=24rem>
    ...
    <div class="flex justify-between">
        <div class="flex w-auto items-center transition rounded-md px-3 py-[7px] bg-black dark:bg-white bg-opacity-10 dark:bg-opacity-10">
            <span class="text-sm">{selected.length} role{selected.length != 1 ? "s" : ""} selected</span>
        </div>
        <div>
            <button class="b-default mr-1" on:click|preventDefault={() => { open = !open; selected = []; }}>Cancel</button>
            <button on:click={() => { if(selected != undefined) { addRole(selected); open = !open; selected = []; } }} disabled={selected.length == 0} class="b-green disabled:opacity-50 disabled:cursor-not-allowed">Add</button>
        </div>
    </div>
</Dialog>