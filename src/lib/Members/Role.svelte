<script lang="ts">
  ...
  let invalidObject: { id: string; invalid: boolean } = {
    id: "",
    invalid: false,
  };

  $: invalid =
    (awaitedRole != undefined &&
      $currentTeam &&
      $currentTeam.level != undefined &&
      awaitedRole.level >= $currentTeam.level) ||
...
{#await role then role}
  {#if role != undefined && role.name != "everyone"}
    <div
      class="inline-flex {invalid
        ? 'bg-red-500 bg-opacity-20 dark:bg-opacity-10 dark:text-red-500 text-red-900'
        : 'bg-black dark:bg-white bg-opacity-10 dark:bg-opacity-10'} p-2 px-3 rounded-lg w-fit items-center gap-2 mb-2 mr-2"
    >
      <div
