<script lang="ts">
  ...
  const teamInfo = getContext("teamInfo") as Writable<
    ...
    <div class="flex flex-row items-center">
      {#if $page.data.kicked}
        <a
          ...
          class="bg-opacity-30 dark:bg-opacity-20 bg-red-500 dark:text-red-500 text-red-900 p-1 px-2.5 text-lg rounded-md mr-2"
          >Kicked</a
        >
      {/if}
      <img
        ...
        alt="profile"
        src={$user.photoURL}
        class="h-10 w-10 min-w-[2.5rem] rounded-full"
      />
    </div>
  </button>
  {#if $open}
    <div
      use:melt={$menu}
      class="bg-backgroud-light w-72 dark:bg-backgroud-dark border-border-light dark:border-border-dark z-[50] shadow-shadow-light dark:shadow-shadow-dark shadow-lg translate-y-2 right-2 box-border border-[1px] rounded-lg flex-col p-2 {$dialogOpen
        ? 'hidden'
        : 'flex'}"
    >
        ...
        <div class="flex flex-row items-center overflow-hidden max-w-full">
          ...
          <div class="ml-3 mr-1 max-w-[11.5rem]">
            <p class="overflow-ellipsis overflow-hidden whitespace-nowrap">
              {$user.displayName}{$user.pronouns != undefined &&
              $user.pronouns != ""
                ? " (" + $user.pronouns + ")"
                : ""}
            </p>
            <p
              class="overflow-ellipsis overflow-hidden whitespace-nowrap opacity-75"
            >
              {$user.email}
            </p>
          </div>
      ...
      <div class="flex flex-row">
        <Tooltip text="Sign Out" class="w-full">
          <button
            ...
            class="hover:cursor-pointer rounded-md b-menu w-full flex justify-around items-center"
          >
        ...
        <Tooltip text="Sign Out of All Devices" class="w-full">
          <button
            ...
            class="hover:cursor-pointer {window.location.pathname ==
            '/settings/profile/'
              ? 'hover:after:opacity-0'
              : ''} rounded-md b-menu w-full flex justify-around items-center"
          >
        ...
        <Tooltip text="Edit Profile" class="w-full">
          <button
            ...
            disabled={window.location.pathname == "/account/profile" ||
              $client?.teams == undefined}
            class="{window.location.pathname == '/account/profile' ||
            $client?.teams == undefined
              ? 'hover:cursor-not-allowed hover:bg-opacity-0 dark:hover:bg-opacity-0 opacity-50 dark:opacity-50'
              : 'hover:cursor-pointer'} rounded-md b-menu w-full flex justify-around items-center"
          >
      ...
      {#if $client != undefined && $client.teams != undefined}
          ...
          {#if $subOpen}
            ...
            >
              {#each $client.teams ?? [] as teamOption}
                ...
                {#if info}
                  <a
                    href="/t/{teamOption.team}"
                    class="rounded-md b-menu flex items-center"
                  >
                    <img
                      class="w-6 h-6 -translate-x-1 rounded-md"
                      src={info.icon}
                    />
        ...
        {:else}
          <NewDialog
            ...
            class="text-left rounded-md b-menu"
          >
            <svelte:fragment slot="button">Switch Team</svelte:fragment>
            <p slot="content">
              {#each $client.teams ?? [] as teamOption}
                {@const info = $teamInfo.get(teamOption.team)}
                {#if info}
                  <a
                    href="/t/{teamOption.team}"
                    class="bg-black dark:bg-white bg-opacity-5 dark:bg-opacity-5 transition p-2 hover:bg-opacity-10 dark:hover:bg-opacity-10 rounded-md w-full h-full flex items-center px-2 mt-2"
                  >
      ...
      {#if $dev}
        <button
          use:melt={$item}
          on:click={() => {
            navigator.clipboard.writeText($client?.uid ?? "NO UID");
          }}
          class="text-left rounded-md b-menu"
        >
          Copy ID
