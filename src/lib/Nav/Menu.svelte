<script lang=ts>
    import { navmenu, navmode } from "$lib/stores";
...
{#if $navmenu}
    <div transition:fly="{{ opacity: 1, duration: 300, easing: quintOut, x: -width}}" class="h-[calc(100dvh-4rem)] bg-slate-100 dark:bg-zinc-900 600 min-w-[300px] w-full bottom-0 left-0 flex flex-col items-center justify-between p-4 z-0">
        <div class="w-full overflow-scroll">
            <div class="w-full">
                <hr>
                <a on:click={() => { $navmenu = false }} href="/">Home</a>
                <hr>
                <a on:click={() => { $navmenu = false }} href="/resources">Resources</a>
                <hr>
                <a on:click={() => { $navmenu = false }} href="/tasks">Tasks</a>
                <hr>
                <a on:click={() => { $navmenu = false }} href="/meetings">Meetings</a>
                <hr>
                <a on:click={() => { $navmenu = false }} href="/reports">Reports</a>
                <hr>
            </div>
        </div>
        <div class="pt-4">
...
<style lang=postcss>
    ...
    a {
        @apply w-full p-4 flex flex-col items-center hover:bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-20 transition rounded-md;
    }
</style>