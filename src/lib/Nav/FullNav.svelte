            ...
            href="{!$global && ($page.params.team || $currentTeam)
              ? '/t/' +
                ($page.params.team == undefined
                  ...
                  : $page.params.team)
              : ''}{link.href}"
            class="transition p-2 mx-1 rounded-md {$page.url.pathname == '/' ||
            $page.url.pathname == '/t/' + $page.data.unverifiedTeam
              ? 'bg-accent-500 text-black'
              : 'bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-5 dark:hover:bg-opacity-5'}"
            >{link.display}</a
          >
        {:else}
          <a
            ...
            class="transition p-2 mx-1 rounded-md {$page.url.pathname ==
              (link.specific ? '/t/' + $team + link.href : link.href) ||
            (link.href != '/' &&
