...
{#key $navigating}
    <div class="flex flex-row ml-2 grow-[1] text-lg font-normal overflow-auto">
        {#each $navLinks as link}
            ...
            {:else} 
                <a href={link.href} class="transition p-2 mx-1 rounded-md {browser && (window.location.pathname == link.href || (link.href != "/" && window.location.pathname.includes(link.href))) ? "bg-accent-light dark:bg-accent-dark text-accent-text-light dark:text-accent-text-dark" : "bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-5 dark:hover:bg-opacity-5"}">{link.display}</a>
            {/if}
            <hr>
        {/each}
    </div>
