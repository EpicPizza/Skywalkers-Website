...
<a class="text-blue-500 leading-7 dark:text-blue-500 hover:underline inline" target="_blank" href={href} on:click={warn} {title}><slot></slot></a>
...
<Dialog width=20rem bind:isOpen={open}>
    ...
    <p class="p-2 mt-4 mb-1 rounded-lg bg-primary-200 dark:bg-primary-800 bg-opacity-25 break-all"><span class="opacity-75">{beginning}</span><span class="font-bold">{domain}</span><span class="opacity-75">{ending}</span></p>

        <div class="flex items-center gap-1 mt-2">
            <button on:click={() => { donotaskagain = !donotaskagain; }} class="p-0.5 {donotaskagain ? "border-blue-700 dark:border-blue-500 bg-blue-200 dark:bg-blue-900 text-blue-700 dark:text-blue-500" : "border-border-light dark:border-border-dark text-border-light dark:text-border-dark"} border-[1px] rounded-md m-0.5 transition disabled:cursor-not-allowed disabled:opacity-40">
    ...
    <div class="flex flex-row-reverse">
        <div class="flex flex-row gap-2">
            ...
            <button class="b-green" on:click={() => { open = !open; window.open(href, "_blank"); check();}}>Continue</button>
        </div>
    </div>
</Dialog>
