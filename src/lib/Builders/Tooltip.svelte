...
<script lang=ts>
    ...
    function check(e: TouchEvent) {
        if(content && e.target && !content.contains(e.target as any) && !e.defaultPrevented) {
            hover = false;
        }
    }
...
<svelte:window on:touchstart={check} bind:innerHeight={windowHeight} bind:innerWidth={windowWidth}></svelte:window>

<!-- svelte-ignore a11y-no-static-element-interactions -->
<div class={style} on:mouseenter={onHover} on:focusin={start} on:focusout={end} on:touchstart={start} on:mouseleave={end} bind:this={content}>
    <slot></slot>
</div>

{#if hover}
    <div style="left: {x}px; bottom: {y}px; width: {width}px" bind:this={tooltip} use:portal={"body"} class="fixed z-[100] flex flex-col items-center overflow-visible">
        {#if bottom}
