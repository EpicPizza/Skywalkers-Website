<script lang="ts">
  ...
  function countdown() {
    ...
    if (diff < 0) {
      ...
      const days =
        (diff - (diff % (1000 * 60 * 60 * 24))) / (1000 * 60 * 60 * 24);
...
<div
  ...
  class="px-4 h-[calc(100dvh)] border-b border-border-light dark:border-border-dark w-full bg-white dark:bg-zinc-800 bg-opacity-70 pattern"
>
  <div
    class="flex flex-col mt-20 justify-around items-center h-[calc(100svh-7.75rem)] w-full {$page
      .url.pathname ==
      '/t/' + $page.data.unverifiedTeam ||
    $page.url.pathname == '/t/' + $page.data.unverifiedTeam + '/verify' ||
    $page.url.pathname == '/team/choose' ||
  ...
  {#if $page.url.pathname == "/t/" + $page.data.unverifiedTeam || $page.url.pathname == "/t/" + $page.data.unverifiedTeam + "/verify"}
    <a
      href="/team/choose"
      class="bg-backgroud-light dark:bg-backgroud-dark border border-border-light dark:border-border-dark px-4 absolute top-[88px] rounded-xl left-4 flex flex-col items-center shadow-sm dark:shadow-lg"
    >
