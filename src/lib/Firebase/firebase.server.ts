...
function getFirebaseAdmin() {
  ...
  const getFirebaseApp = (prod = false): admin.app.App => {
    if (prod && DEV == "TRUE") {
      if (prodApp == undefined) {
        ...
        for (var i = 0; i < admin.apps.length; i++) {
          if (
            admin.apps[i] != null &&
            (admin.apps[i] as admin.app.App).name == "Server-Prod"
          ) {
          ...
          prodApp = admin.initializeApp(
            {
              credential: admin.credential.cert(
                JSON.parse(PROD_ADMIN) as admin.ServiceAccount,
              ),
    ...
    if (app == undefined) {
      //this get reruns on every change durring preview, but firebase admin still sees the pervious instance made, so this just checks if we can use a previous firebase instance, otherwise it will cause an error because firebase thinks we are reintializing
      ...
      if (DEV == "TRUE") {
        if (admin.apps == null) {
          ...
          for (var i = 0; i < admin.apps.length; i++) {
            if (
              admin.apps[i] != null &&
              (admin.apps[i] as admin.app.App).name == "Server"
            ) {
...
export async function getPhotoURL(url: string, id: string) {
  ...
  if (info == undefined)
    throw error(501, "Unable to determine profile picture file extension.");
...
async function initFirstore(firestore: Firestore) {
  const ref = firestore
    .collection("teams")
    .doc("111111")
    .collection("meetings"); //fine since just in development

  ref.onSnapshot(async (snapshot) => {
    console.log("checking for calendar events");

    const docs = Array.from(snapshot.docChanges(), (change) =>
      change.type == "modified" ? change.doc : undefined,
    );
