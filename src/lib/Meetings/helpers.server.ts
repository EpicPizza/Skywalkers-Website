...
export interface FetchedMeeting
  extends Omit<Meeting, "synopsis" | "mentor" | "lead" | "role" | "signups"> {
...
export interface FetchedPromisedMeeting
  extends Omit<Meeting, "synopsis" | "mentor" | "lead" | "role" | "signups"> {
...
export async function getTeamNotion(
  team: string,
): Promise<{ secret: string; database: string }> {
  ...
  if (!doc.exists || doc.data() == undefined)
    throw new Error("Notion settings not found.");

  return {
    secret: doc.data()?.secret as string,
    database: doc.data()?.database as string,
  };
}

const update = {
  vthreezzero: async function (
    meeting: Meeting,
    user: string,
    team: string,
  ): Promise<Meeting> {
...
export async function getMeeting(
  ...
  if (data == undefined)
    throw { message: "Meeting Not Found", type: "display" };
...
export async function getSynopsis(id: string, user: string, team: string) {
  ...
  if (synopsis == undefined)
    throw { message: "Synopsis not found.", type: "display" };
...
export async function getSynopses(
  ref: Query,
  members: Map<string, SecondaryUser>,
) {
  const docs = (await ref.get()).docs;

  const synopses = new Array<{
    body: string;
    hours: { member: SecondaryUser; time: number }[];
    attachments: {
      url: string;
      type: string;
      name: string;
  ...
  for (let i = 0; i < docs.length; i++) {
    ...
    for (let i = 0; i < synopsis.hours.length; i++) {
      let member =
        members.get(synopsis.hours[i].member.id) ??
        getDefault(synopsis.hours[i].member.id);
...
export async function getFetchedMeetings(
  ref: Query,
  user: string,
  team: string,
) {
  ...
  for (let i = 0; i < rawMeetings.length; i++) {
    meetings.push({
      ...
      lead: (await getMember(
        rawMeetings[i].lead.id,
        team,
        true,
      )) as SecondaryUser,
      synopsis:
        rawMeetings[i].synopsis == null
          ? null
          : (await getMember(
              (rawMeetings[i].synopsis as DocumentReference).id,
              team,
              true,
            )) ?? null,
      mentor:
        rawMeetings[i].mentor == null
          ? null
          : (await getMember(
              (rawMeetings[i].mentor as DocumentReference).id,
              team,
              true,
            )) ?? null,
      ...
      role: rawMeetings[i].role
        ? (await getRole(rawMeetings[i].role as string, team)) ?? null
        : null,
      link: rawMeetings[i].link,
      id: rawMeetings[i].id as string,
      version: rawMeetings[i].version,
      signups: await Promise.all(
        Array.from(rawMeetings[i].signups, (signup) => {
          return getMember(signup, team);
        }),
      ),
...
export async function getFetchedMeetingsOptimized(
  ref: Query,
  user: string,
  team: string,
  members: Map<string, SecondaryUser>,
  ...
  for (let i = 0; i < rawMeetings.length; i++) {
    ...
    meetings.push({
      ...
      synopsis:
        rawMeeting.synopsis == null
          ? null
          : members.get(rawMeeting.synopsis.id) ??
            getDefault(rawMeeting.synopsis.id),
      mentor:
        rawMeeting.mentor == null
          ? null
          : members.get(rawMeeting.mentor.id) ??
            getDefault(rawMeeting.mentor.id),
      ...
      role: rawMeeting.role
        ? roles.get(rawMeeting.role) ?? getDefaultRole(rawMeeting.role)
        : null,
      link: rawMeeting.link,
      id: rawMeeting.id as string,
      version: rawMeeting.version,
      signups: Array.from(rawMeeting.signups, (signup) => {
        return members.get(signup) ?? getDefault(signup);
      }),
...
export async function getFetchedMeeting(
  id: string,
  user: string,
  team: string,
): Promise<FetchedMeeting> {
  ...
  const meeting = {
    ...
    synopsis:
      rawMeeting.synopsis == null
        ? null
        : (await getMember(rawMeeting.synopsis.id, team, true)) ?? null,
    mentor:
      rawMeeting.mentor == null
        ? null
        : (await getMember(rawMeeting.mentor.id, team, true)) ?? null,
    location: rawMeeting.location as string,
    starts: rawMeeting.starts,
    ends: rawMeeting.ends,
    thumbnail: rawMeeting.thumbnail,
    ...
    update: rawMeeting.update,
    role: rawMeeting.role
      ? (await getRole(rawMeeting.role, team)) ?? null
      : null,
    link: rawMeeting.link,
    id: id as string,
    version: rawMeeting.version,
    signups: await Promise.all(
      Array.from(rawMeeting.signups, (signup) => {
        return getMember(signup, team, true);
      }),
    ),
    virtual: rawMeeting.virtual,
    calendar: rawMeeting.calendar,
...
export async function createMeetingFromForm(
  form: SuperValidated<typeof meetingSchema>,
  user: string,
  team: string,
) {
  const db = firebaseAdmin.getFirestore();
  ...
  const meeting = await createMeeting(
    {
      ...
      mentor:
        form.data.mentor == undefined || form.data.mentor == ""
          ? null
          : db.collection("users").doc(form.data.mentor),
      synopsis:
        form.data.synopsis == undefined || form.data.synopsis == ""
          ? null
          : db.collection("users").doc(form.data.synopsis),
...
export async function editMeetingFromForm(
  ...
  await editMeeting(
    currentMeeting,
    {
      ...
      lead: db.collection("users").doc(form.data.lead),
      mentor:
        form.data.mentor == null || form.data.mentor == ""
          ? null
          : db.collection("users").doc(form.data.mentor),
      synopsis:
        form.data.synopsis == null || form.data.synopsis == ""
          ? null
          : db.collection("users").doc(form.data.synopsis),
      starts: form.data.starts,
      ends: form.data.ends,
...
export async function validateFormMeeting(
  form: SuperValidated<typeof meetingSchema>,
  user: string,
  team: string,
) {
  ...
  if (form.data.ends.valueOf() <= form.data.starts.valueOf())
    return message(form, "Start time must be before end time.");
  ...
  if (
    !users.includes(form.data.lead) ||
    (form.data.mentor != null &&
      form.data.mentor != "" &&
      !users.includes(form.data.mentor)) ||
    (form.data.synopsis != null &&
      form.data.synopsis != "" &&
      !users.includes(form.data.synopsis))
  ) {
  ...
  if (
    !(form.data.role == undefined || form.data.role == "") &&
    !(
      await db
        .collection("teams")
        .doc(team)
    ...
    return message(form, "Role not found.");
...
export async function createMeeting(
  ...
  if (client == undefined) {
    try {
      ...
      throw {
        message:
          "Google calendar integration down. Discord bot down. Please contact support, something has gone really wrong.",
        type: "display",
      };
    }
        ...
        rich_text: [
          {
            ...
            text: {
              content:
                (
                  await getRole(
                    role == "" || role == undefined ? "000000" : role,
                    team,
                  )
                )?.name ?? "Blank",
            },
        ...
        rich_text: [
          {
            ...
            text: {
              content: mentor
                ? (await getMember(mentor.id, team)).displayName ?? "None"
                : "None",
            },
          },
        ],
        ...
        rich_text: [
          {
            ...
            text: {
              content: synopsis
                ? (await getMember(synopsis.id, team)).displayName ?? "None"
                : "None",
            },
          },
        ],
        ...
        rich_text: [
          {
            ...
            text: {
              content:
                "EDIT IN WEBSITE, your changes here will be overwritten.",
            },
          },
        ],
      },
...
export async function createMeetingFromSchedule(
  {
    name,
    location,
    starts,
...
export async function completeMeeting(
  ...
  synopsis:
    | {
        ...
        hours: { member: DocumentReference; time: number }[];
        synopsis: string;
      }
    | undefined = undefined,
) {
  ...
  if (complete == false) {
    ...
    await notion.blocks.children.append({
      block_id: id,
      children: (
        markdownToBlocks(synopsis.synopsis) as unknown as BlockObjectRequest[]
      ).concat(attachments),
    });
  }
}
...
export async function editSynopsis(
      ...
      if (attachmentHelpers.isImage(synopsis.attachments[i].type)) {
        ...
        attachments.push({
          ...
          file: {
            ...
            name:
              synopsis.attachments[i].name + "." + synopsis.attachments[i].ext,
          },
        });
      }
    }
  ...
  await notion.blocks.children.append({
    ...
    children: (
      markdownToBlocks(
        "synopsis" in synopsis ? synopsis.synopsis : synopsis.body,
...
export async function removeGuestSignup(
  signUp: string,
  id: string,
  team: string,
) {
...
export async function sendConfirmations(
  ...
  const hasConfirmations = await db.runTransaction(async (t) => {
    const data = (
      await t.get(
        db.collection("teams").doc(team).collection("meetings").doc(meeting.id),
      )
    ).data();
  ...
  if (emailTo.length > 0) {
    const members = await firebaseAdmin
      .getAuth()
      .getUsers(Array.from(emailTo, (signup) => ({ uid: signup })));
...
export async function markConfirmation(
  ...
  await db.runTransaction(async (t) => {
    ...
    ).data()?.confirmations as Meeting["confirmations"];
