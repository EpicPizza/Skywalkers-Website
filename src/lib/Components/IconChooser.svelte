...
<input type="text" bind:value {name} class={style} />
<button
...
<Dialog bind:isOpen={open} bind:initialFocus>
  ...
  <div slot="description" class="flex mt-4 mb-4">
    ...
    />
    <button
      class="px-2 h-[2.125rem] ml-2 bg-blue-500 text-white dark:text-white rounded-md hover:opacity-95 transition"
      on:click={() => {
        filter("");
        searching = "";
      }}
      id="iconsearchbutton">Clear</button
    >
  </div>
  ...
  <div
    ...
    bind:this={container}
    on:scroll={scroll}
    class="h-[calc(100dvh-17rem)] /* <<< skull emoji */ overflow-auto overflow-y-visible pb-2"
  >
      ...
      <div class="inline-block mr-2 mt-2">
        <Tooltip text={icon}>
          <button
            ...
            class="p-2 bg-black dark:bg-white {selected == icon
              ? 'bg-opacity-20'
              : 'bg-opacity-5'} {selected == icon
              ? 'dark:bg-opacity-20'
              : 'dark:bg-opacity-5'} hover:bg-opacity-20 dark:hover:bg-opacity-20 rounded-sm transition"
          >
            <Icon scale="2rem" {icon} />
          </button>
        </Tooltip>
      </div>
    {:else}
      <p class="text-center mt-4">Nothing Found</p>
    {/each}
    {#if loader && range != -1}
      <div
        transition:fly={{ y: 20 }}
        id="loading"
        class="h-12 w-12 absolute bottom-16 bg-backgroud-light dark:bg-backgroud-dark border-[1px] border-border-light dark:border-border-dark rounded-md left-1/2 -translate-x-1/2 flex items-center justify-around"
      >
        <Loading></Loading>
      </div>
    {/if}
  ...
  <div class="w-full flex flex-row-reverse mt-4 justify-between">
    ...
    <div>
      {#if selected != ""}
        <div
          class="flex items-center h-[2.125rem] bg-zinc-200 dark:bg-zinc-600 rounded-md"
        >
          <p class="ml-2">Selected:</p>
          <Tooltip text={selected}>
            <div
              class="h-[2.125rem] w-[2.125rem] flex items-center justify-around"
            >
