...
<div
  ...
  class="border-border-light dark:border-border-dark border-[1px] rounded-xl p-4 transition-all bg-blue-500 {draggingover
    ? 'outline-2 outline outline-blue-500 -outline-offset-1 bg-opacity-10'
    : 'outline-4 outline outline-transparent -outline-offset-1 bg-opacity-0'}"
>
  <div class="flex flex-col items-center py-4">
    ...
    <button
      ...
      class="opacity-75">or upload a file</button
    >
  </div>
  ...
  {/if}
  <div
    class="flex flex-col gap-3 {!($uploads.length == 0 && old.length == 0)
      ? 'mb-1 mt-4'
      : ''}"
  >
    {#if old.length != 0}
      {#each old as attachment}
        <div
          class="flex items-center {attachment.remove
            ? 'opacity-50'
            : 'opacity-100'}"
        >
          <Icon
            scale="1.5rem"
            class="mx-2 mr-3"
            icon={attachment.image == "true" ? "image" : "draft"}
          ></Icon>
          <div class="w-[calc(100%-2.75rem)]">
            <div class="flex items-center justify-between">
              <p
                class="max-w-[80%] whitespace-nowrap overflow-hidden overflow-ellipsis"
              >
                {attachment.name}
              </p>
              <div class="flex items-center gap-1">
                <button
                  ...
                  class="w-6 h-6 bg-black dark:bg-white rounded-full transition-all bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 flex items-center justify-around"
                >
    ...
    {#each $uploads as upload (upload.id)}
      <div animate:flip class="flex items-center">
        <Icon
          scale="1.5rem"
          class="mx-2 mr-3"
          icon={upload.type.image ? "image" : "draft"}
        ></Icon>
        <div class="w-[calc(100%-2.75rem)]">
          <div class="flex items-center justify-between">
            <p
          ...
          {#if upload.finished}
            <p class="text-xs opacity-75">
              {upload.type.ext} - {filesize(upload.size, { standard: "jedec" })}
            </p>
