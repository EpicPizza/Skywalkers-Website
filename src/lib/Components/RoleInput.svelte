...
<button class="-ml-[1.5rem] -translate-x-[0.375rem]" on:click|preventDefault={handleClick}>
...
<Dialog isOpen={open} width=24rem>
    ...
    <div class="flex justify-between">
        ...
        <div>
            <button class="b-default mr-1" on:click|preventDefault={() => { open = !open; selected = display; }}>Cancel</button>
            <button on:click={() => { if(selected != undefined) { value = selected.id; display = selected; open = !open; } else { value = ""; display = undefined; open = !open; } }} class="b-green disabled:opacity-50 disabled:cursor-not-allowed">Choose</button>
        </div>
    </div>
</Dialog>