<script lang=ts>
    ...
    const onInput = (e: Event) => { date = trytoparse((e.target as any).value); console.log((e.target as any).value) }
</script>

<input on:keypress={(e) => { if(e.key == 'Enter') { dispatch("enter") } }} bind:this={element} name={name} type="date" value={stringdate} on:input={onInput} class={style}/>