<script lang="ts">
  ...
  beforeNavigate((event) => {
    if (
      tainted &&
      !confirm(
        "You have unsaved permission changes! Are you sure want to leave this page?",
      )
    ) {
      event.cancel();
    }
...
{#if !everyone}
  <DeleteDialog bind:open={openDelete} {role} on:delete={handleDelete}
  ></DeleteDialog>
...
<div bind:this={page} class="w-full relative">
  <div style="background-color: {role.color};" class="w-full h-24 relative">
    {#if !(!$currentPermissions.includes("MANAGE_ROLES") || $currentTeam == undefined || $currentTeam.level == undefined || $currentTeam.level <= role.level)}
      <button
        ...
        class="m-2 p-2 {everyone
          ? 'hidden'
          : 'block'} absolute right-0 top-0 rounded-full bg-opacity-10 hover:bg-opacity-20 transition {mode
          ? 'text-black bg-black'
          : 'text-white bg-white'}"
      >
      ...
      <button
        ...
        class="m-2 p-2 absolute right-12 {everyone
          ? 'hidden'
          : 'block'} top-0 rounded-full bg-opacity-10 hover:bg-opacity-20 transition {mode
          ? 'text-black bg-black'
          : 'text-white bg-white'}"
      >
        <Icon icon="edit"></Icon>
    ...
    <button
      ...
      class="m-2 p-2 absolute {!$currentPermissions.includes('MANAGE_ROLES') ||
      $currentTeam == undefined ||
      $currentTeam.level <= role.level
        ? 'right-0'
        : 'right-24'} {everyone
        ? 'hidden'
        : 'block'} top-0 rounded-full {openList
        ? 'bg-opacity-20'
        : 'bg-opacity-10'} hover:bg-opacity-20 transition {mode
        ? 'text-black bg-black'
        : 'text-white bg-white'}"
    >
    ...
    {#if $dev}
      <a
        ...
        href="https://console.firebase.google.com/u/0/project/frc-skywalkers{env
          ? '-dev'
          : ''}/firestore/data/~2Fteams~2F{$team}~2Froles~2F{role.id}"
        class="m-2 p-2 absolute {!$currentPermissions.includes(
          'MANAGE_ROLES',
        ) ||
        $currentTeam == undefined ||
        ...
        $currentTeam.level <= role.level
          ...
          : everyone
            ? 'right-0'
            : 'right-36'} top-0 rounded-full bg-opacity-10 hover:bg-opacity-20 transition {mode
          ? 'text-black bg-black'
          : 'text-white bg-white'}"
      >
  ...
  <div class="p-8 pb-3 pt-5">
    ...
    {#if everyone}
      <p class="mb-6">
        Anyone who is verified automatically gets the permissions given in this
        section. This role cannot be renamed, deleted, or moved, and this role
        is not visible on profiles.
      </p>
    {:else}
      <p class="mb-6">
        Depending on permissions, members may or may not be able to perform
        actions on higher level members.
      </p>
    {/if}
    ...
    <RolePermission permission="MANAGE_CODES" bind:permissions {role}>
      <svelte:fragment slot="title">Manage Codes</svelte:fragment>
      <svelte:fragment slot="description">
        Members will be able to generate (and delete) codes that new users can
        use to join the team.
      </svelte:fragment>
    </RolePermission>

    <RolePermission permission="KICK_MEMBERS" bind:permissions {role}>
      <svelte:fragment slot="title">Kick Members</svelte:fragment>
      <svelte:fragment slot="description">
    ...
    <RolePermission permission="VIEW_LOGS" bind:permissions {role}>
      <svelte:fragment slot="title">View Logs</svelte:fragment>
      <svelte:fragment slot="description">
        Members will be able to see all logs, which include meetings, synopses,
        roles, and more.
      </svelte:fragment>
    </RolePermission>
    ...
    <RolePermission permission="MANAGE_ROLES" bind:permissions {role}>
      <svelte:fragment slot="title">Manage Roles</svelte:fragment>
      <svelte:fragment slot="description">
        Members will be able to edit/create/delete/add roles below their level,
        this is dangerous since they can give themselves any permission.
      </svelte:fragment>
    </RolePermission>
    ...
    <RolePermission permission="VIEW_MEETINGS" bind:permissions {role}>
      <svelte:fragment slot="title">View Meetings</svelte:fragment>
      <svelte:fragment slot="description">
        Members will be able to view all available meetings and sign up up for
        them.
      </svelte:fragment>
    </RolePermission>

    <RolePermission permission="EDIT_MEETINGS" bind:permissions {role}>
      <svelte:fragment slot="title">Edit Meetings</svelte:fragment>
      <svelte:fragment slot="description">
    ...
    <RolePermission permission="CREATE_MEETINGS" bind:permissions {role}>
      <svelte:fragment slot="title">Create Meetings</svelte:fragment>
      <svelte:fragment slot="description">
        Members will be able to create upcoming meetings. They will also be able
        to edit/delete meetings they create.
      </svelte:fragment>
    </RolePermission>

    <RolePermission permission="DELETE_MEETINGS" bind:permissions {role}>
      <svelte:fragment slot="title">Delete Meetings</svelte:fragment>
      <svelte:fragment slot="description">
    ...
    <RolePermission permission="LEAVE_SIGNUP" bind:permissions {role}>
      <svelte:fragment slot="title">Leave Signups</svelte:fragment>
      <svelte:fragment slot="description">
        Without this permission, members will not be able to remove themselves
        if they sign up for a meeting.
      </svelte:fragment>
    </RolePermission>

    <RolePermission permission="MODERATE_MEETINGS" bind:permissions {role}>
      <svelte:fragment slot="title">Moderate Meetings</svelte:fragment>
      <svelte:fragment slot="description">
    ...
    <RolePermission permission="ADD_SYNOPSES" bind:permissions {role}>
      <svelte:fragment slot="title">Add Synopses</svelte:fragment>
      <svelte:fragment slot="description">
        Members will be able to add a synopsis to any meeting, also marking them
        complete.
      </svelte:fragment>
    </RolePermission>

    <RolePermission permission="EDIT_SYNOPSES" bind:permissions {role}>
      <svelte:fragment slot="title">Edit Synopses</svelte:fragment>
      <svelte:fragment slot="description">
    ...
    <RolePermission permission="MODERATE_SYNOPSES" bind:permissions {role}>
      <svelte:fragment slot="title">Moderate Synopses</svelte:fragment>
      <svelte:fragment slot="description">
        Members will be able to reject/delete a synopsis to any meeting, also
        marking them as active again.
      </svelte:fragment>
    </RolePermission>
  </div>
  {#if tainted || loadingSave}
    <div
      transition:fly={{ easing: cubicInOut, y: 100 }}
      class="sticky bottom-0 w-full pointer-events-none p-5 pt-0"
    >
      <div
        class="p-3 w-full flex items-center justify-between rounded-lg pointer-events-auto bg-zinc-100 dark:bg-zinc-900 border-border-light dark:border-border-dark border-[1px] shadow-shadow-light dark:shadow-shadow-dark shadow-lg"
      >
        ...
        <div class="flex">
          ...
          <button
            on:click={handleSave}
            class="b-primary ml-2 w-[53.5px] flex justify-around"
          >
            {#if loadingSave}
