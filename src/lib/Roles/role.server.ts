...
export async function getSpecifiedRoles(
...
export async function getSpecifiedRolesWithTransaction(
  refs: DocumentReference[],
...
async function getMembersFromTransaction(
  ...
  for (let i = 0; i < ids.length; i++) {
    const user = (
      await transaction.get(
        firebaseAdmin.getFirestore().collection("users").doc(ids[i].id),
      )
    ).data() as any;
...
export async function getRolesWithTransaction(
  team: string,
  transaction: Transaction,
  ...
  roleDocs.forEach(async (doc) => {
    roles.push({
      ...
      connectTo: doc.data().connectTo as string | null,
      members: withMembers
        ? await getMembersFromTransaction(
            doc.data().members as DocumentReference[],
            transaction,
          )
        : [],
      id: doc.id,
