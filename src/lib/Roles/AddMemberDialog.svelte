...
<Dialog bind:isOpen={open} {initialFocus}>
  ...
  <div
    slot="content"
    class="min-h-[calc(100dvh-17rem)] overflow-auto overflow-y-visible"
  >
    {#await usersPromise}
    ...
    {:then users}
      {#each users as user}
        ...
        {#if (search == "" || user.displayName
            .toLowerCase()
            .includes(search.toLowerCase())) && checkIfAlreadyHas(user.id)}
          <button
            on:click={() => {
            ...
            class="mt-2 flex items-center p-2 bg-black dark:bg-white {selected.includes(
              user.id,
            )
              ? 'dark:bg-opacity-20 bg-opacity-20'
              : 'dark:bg-opacity-5 bg-opacity-5'} w-full transition rounded-lg"
          >
            <img
      ...
      {:else}
        {#if areThereAny}
          <div
            class="w-full h-16 flex items-center justify-around text-red-500 font-bold"
          >
            No Users Found
          </div>
        {/if}
  ...
  <div class="flex flex-row-reverse justify-between">
    <div class="flex flex-row-reverse">
      <button
        ...
        class="b-green {selected.length == 0
    ...
    >
      <p class="text-sm">
        {selected.length} member{selected.length == 1 ? "" : "s"} choosen
      </p>
    </div>
  </div>
</Dialog>
