      ...
      if ($message.length > "Role Added at ".length) {
        goto(
          "/t/" +
            $team +
            "/settings/roles/" +
            $message.substring("Role Added at ".length, $message.length),
        );
      }
...
<Dialog bind:isOpen={open} width="24rem" bind:initialFocus>
  ...
  <div slot="content" class="mt-4">
    <form method="POST" use:enhance action="/t/{$team}/api/roles/create">
      <!-- No actions allowed on layouts, so I instead put the action in a seperate url. -->
      ...
      <Error {message} disallowMessage="Role Added" {allErrors}></Error>
