<script lang="ts">
  ...
  async function loadRoles(value: boolean) {
    ...
    if (
      new Date().valueOf() - timestamp.valueOf() > 1000 * 60 * 5 ||
      (connectedTo == -1 && role.connectTo != null)
    ) {
...
<button
  ...
  disabled={!$permissions.includes("MANAGE_ROLES") ||
    $currentTeam == undefined ||
    $currentTeam.level == undefined ||
    $currentTeam.level <= role.level}
  class="bg-zinc-200 dark:bg-zinc-600 p-2 px-3 rounded-lg hover:opacity-90 disabled:hover:opacity-100 disabled:cursor-not-allowed trnasition disabled:opacity-40 hover:disabled:opacity-40"
>
  ...
  {:else}
    {#await rolesPromise}
      <div class="flex justify-around">
        <div
          class="w-6 h-6 border-[2px] border-transparent dark:border-transparent border-t-text-light rounded-full dark:border-t-text-dark animate-spin"
        ></div>
      </div>
    {:then roles}
      {#if connectedTo != -1}
        <div class="flex items-center">
          <div
            style="background-color: {roles[connectedTo].color};"
            class="w-5 h-5 mr-1.5 rounded-full"
          ></div>
    ...
    {:then}
      {#if role.connectTo != null}
        <button
          ...
          class="flex w-full items-center transition rounded-md p-2 mb-1 bg-black dark:bg-white bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10"
        >
      ...
      {#each roles as discordRole, i}
        ...
        >
          <div
            style="background-color: {discordRole.color};"
  ...
  <div class="flex justify-between">
    <div>
      {#if selected != undefined && selected > -1}
        ...
        >
          <div
            style="background-color: {roles[selected].color};"
    ...
    <div>
      ...
      <button
        ...
        >{selected == -1 ? "Save" : "Connect"}</button
      >
    </div>
  </div>
</Dialog>
