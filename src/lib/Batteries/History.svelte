...
{#if open && $client != undefined}
  <div tabindex="-1" transition:slide class="w-full overflow-auto pt-4">
    ...
    <div class="max-h-52 overflow-auto w-full">
      <div
        class="flex sm:items-center flex-col sm:flex-row gap-2 sm:gap-1.5 pr-0.5 mb-4 pt-1"
      >
        <div class="flex items-center gap-1.5 pr-0.5 min-w-fit ml-1 sm:ml-0">
          <img
            class="w-7 h-7 rounded-full"
            alt="{$client.displayName}{$client.pronouns
              ? ' (' + $client.pronouns + ')'
              : ''}"
            src={$client.photoURL}
          />
          <span class="whitespace-nowrap"
            >{$client.displayName}{$client.pronouns
              ? " (" + $client.pronouns + ")"
              : ""}:</span
          >
        </div>
        <input
          ...
          class="rounded-md w-[calc(100%-2px)] p-1 m-0.5"
      ...
      <div class="flex flex-col gap-2">
        {#each battery.history as message}
          <Member id={message.author} let:member>
            <div
              class="flex flex-col sm:flex-row items-start gap-3 border border-border-light dark:border-border-dark rounded-lg p-4 sm:p-0 sm:border-none"
            >
              {#await member}
              ...
              {:then member}
                <div class="flex items-center gap-1.5 min-w-fit">
                  <img
                    class="w-7 h-7 rounded-full"
                    alt="{member.displayName}{member.pronouns
                      ? ' (' + member.pronouns + ')'
                      : ''}"
                    src={member.photoURL}
                  />
              ...
              {/await}
              <p class="mt-[2px] ml-1">
                {message.message}<span class="ml-2 text-xs opacity-50"
                  >({dnt.format(message.timestamp, "MM/DD/YYYY")})</span
                >
              </p>
            </div>
          </Member>
