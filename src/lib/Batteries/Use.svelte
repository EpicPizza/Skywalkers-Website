...
<Menu
  choices={["Competition", "Practice", "Testing", "Decommissioned", "Unknown"]}
  selected={battery.use}
  on:select={(e) => {
    changeUse(e.detail);
  }}
>
  <div
    class="h-8 sm:h-fit flex sm:block flex-col justify-around items-center whitespace-nowrap rounded-lg px-1 sm:px-0 sm:bg-transparent dark:sm:bg-transparent bg-black dark:bg-white bg-opacity-10 dark:bg-opacity-10 w-full"
  >
    <span>
      Use:
      {#if battery.use == "Competition"}
        <span class="text-green-600 dark:text-green-400 font-extrabold"
          >Competition</span
        >
      {:else if battery.use == "Practice"}
        <span class="text-yellow-600 dark:text-yellow-400 font-extrabold"
          >Practice</span
        >
      ...
      {:else if battery.use == "Decommissioned"}
        <span class="text-red-600 dark:text-red-400 font-extrabold"
          >Decommissioned</span
        >
