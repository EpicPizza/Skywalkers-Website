...
export async function finish(queue: Queue) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    streams.push(
      bucket.file(files[i]).createReadStream({ start: 146, end: size - 47 }),
    );
  }
  ...
  const typeStream = await fileTypeStream(
    combinedStream.pipe(new PassThrough()),
  );
  ...
  await ref.update({
    ...
    type: type,
    name:
      queue.name.length > type.ext.length + 1
        ? queue.name.substring(0, queue.name.length - type.ext.length - 1)
        : queue.name,
  });
}
...
export async function addChunk(marker: NewQueue, chunk: Chunk) {
  ...
  if (queue.chunks > 41) {
    throw {
      message: "Too many chunks, indicates an above limit file size.",
      type: "display",
    };
  }
